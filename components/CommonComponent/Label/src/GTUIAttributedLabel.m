...
#define kGTUILineBreakWordWrapTextWidthScalingFactor (M_PI / M_E)


static CGFloat const GTUIFLOAT_MAX = 100000;

NSString * const kGTUIStrikeOutAttributeName = @"GTUIStrikeOutAttribute";
NSString * const kGTUIBackgroundFillColorAttributeName = @"GTUIBackgroundFillColor";
NSString * const kGTUIBackgroundFillPaddingAttributeName = @"GTUIBackgroundFillPadding";
NSString * const kGTUIBackgroundStrokeColorAttributeName = @"GTUIBackgroundStrokeColor";
NSString * const kGTUIBackgroundLineWidthAttributeName = @"GTUIBackgroundLineWidth";
NSString * const kGTUIBackgroundCornerRadiusAttributeName = @"GTUIBackgroundCornerRadius";

const NSTextAlignment GTUITextAlignmentLeft = NSTextAlignmentLeft;
const NSTextAlignment GTUITextAlignmentCenter = NSTextAlignmentCenter;
const NSTextAlignment GTUITextAlignmentRight = NSTextAlignmentRight;
const NSTextAlignment GTUITextAlignmentJustified = NSTextAlignmentJustified;
const NSTextAlignment GTUITextAlignmentNatural = NSTextAlignmentNatural;

const NSLineBreakMode GTUILineBreakByWordWrapping = NSLineBreakByWordWrapping;
const NSLineBreakMode GTUILineBreakByCharWrapping = NSLineBreakByCharWrapping;
const NSLineBreakMode GTUILineBreakByClipping = NSLineBreakByClipping;
const NSLineBreakMode GTUILineBreakByTruncatingHead = NSLineBreakByTruncatingHead;
const NSLineBreakMode GTUILineBreakByTruncatingMiddle = NSLineBreakByTruncatingMiddle;
const NSLineBreakMode GTUILineBreakByTruncatingTail = NSLineBreakByTruncatingTail;

typedef NSTextAlignment GTUITextAlignment;
typedef NSLineBreakMode GTUILineBreakMode;
...
static inline CGFloat GTUIFlushFactorForTextAlignment(NSTextAlignment textAlignment) {
    switch (textAlignment) {
        case GTUITextAlignmentCenter:
            return 0.5f;
        case GTUITextAlignmentRight:
            return 1.0f;
        case GTUITextAlignmentLeft:
        default:
            return 0.0f;
    }
}

static inline NSDictionary * NSAttributedStringAttributesFromLabel(GTUIAttributedLabel *label) {
...
static inline CGSize CTFramesetterSuggestFrameSizeForAttributedStringWithConstraints(CTFramesetterRef framesetter, NSAttributedString *attributedString, CGSize size, NSUInteger numberOfLines) {
    ...
    CGSize constraints = CGSizeMake(size.width, GTUIFLOAT_MAX);

    if (numberOfLines == 1) {
        // If there is one line, the size that fits is the full width of the line
        constraints = CGSizeMake(GTUIFLOAT_MAX, GTUIFLOAT_MAX);
    } else if (numberOfLines > 0) {
        // If the line count of the label more than 1, limit the range to size to the number of lines that have been set
        CGMutablePathRef path = CGPathCreateMutable();
        CGPathAddRect(path, NULL, CGRectMake(0.0f, 0.0f, constraints.width, GTUIFLOAT_MAX));
...
@interface GTUIAccessibilityElement : UIAccessibilityElement
@property (nonatomic, weak) UIView *superview;
@property (nonatomic, assign) CGRect boundingRect;
@end

@implementation GTUIAccessibilityElement
...
@property (readwrite, nonatomic, strong) GTUIAttributedLabelLink *activeLink;
@property (readwrite, nonatomic, strong) NSArray *accessibilityElements;

- (void) longPressGestureDidFire:(UILongPressGestureRecognizer *)sender;
@end

@implementation GTUIAttributedLabel {
@private
...
- (void)addLink:(GTUIAttributedLabelLink *)link {
    [self addLinks:@[link]];
}
...
- (void)addLinks:(NSArray *)links {
    ...
    for (GTUIAttributedLabelLink *link in links) {
        if (link.attributes) {
...
- (GTUIAttributedLabelLink *)addLinkWithTextCheckingResult:(NSTextCheckingResult *)result
                                  ...
                                  attributes:(NSDictionary *)attributes
{
    ...
    for (NSTextCheckingResult *result in results) {
        ...
        GTUIAttributedLabelLink *link = [[GTUIAttributedLabelLink alloc] initWithAttributes:attributes
...
- (GTUIAttributedLabelLink *)addLinkToURL:(NSURL *)url
                               withRange:(NSRange)range
{
    return [self addLinkWithTextCheckingResult:[NSTextCheckingResult linkCheckingResultWithRange:range URL:url]];
}

- (GTUIAttributedLabelLink *)addLinkToAddress:(NSDictionary *)addressComponents
                                   withRange:(NSRange)range
{
...
- (GTUIAttributedLabelLink *)addLinkToPhoneNumber:(NSString *)phoneNumber
                                       withRange:(NSRange)range
{
...
- (GTUIAttributedLabelLink *)addLinkToDate:(NSDate *)date
                                withRange:(NSRange)range
{
...
- (GTUIAttributedLabelLink *)addLinkToTransitInformation:(NSDictionary *)components
                                              withRange:(NSRange)range
{
...
- (GTUIAttributedLabelLink *)linkAtPoint:(CGPoint)point {
    ...
    GTUIAttributedLabelLink *result = [self linkAtCharacterIndex:[self characterIndexAtPoint:point]];
...
- (GTUIAttributedLabelLink *)linkAtRadius:(const CGFloat)radius aroundPoint:(CGPoint)point {
    ...
    GTUIAttributedLabelLink *link = nil;
...
- (GTUIAttributedLabelLink *)linkAtCharacterIndex:(CFIndex)idx {
...
- (CFIndex)characterIndexAtPoint:(CGPoint)p {
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        CGFloat flushFactor = GTUIFlushFactorForTextAlignment(self.textAlignment);
                ...
                context:(CGContextRef)c
{
    ...
    CFArrayRef lines = CTFrameGetLines(frame);
    NSInteger numberOfLines = self.numberOfLines > 0 ? MIN(self.numberOfLines, CFArrayGetCount(lines)) : CFArrayGetCount(lines);
    BOOL truncateLastLine = (self.lineBreakMode == GTUILineBreakByTruncatingHead || self.lineBreakMode == GTUILineBreakByTruncatingMiddle || self.lineBreakMode == GTUILineBreakByTruncatingTail);
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        if (lineIndex == numberOfLines - 1 && truncateLastLine) {
            ...
            if (!(lastLineRange.length == 0 && lastLineRange.location == 0) && lastLineRange.location + lastLineRange.length < textRange.location + textRange.length) {
                ...
                GTUILineBreakMode lineBreakMode = self.lineBreakMode;

                // Multiple lines, only use UILineBreakModeTailTruncation
                if (numberOfLines != 1) {
                    lineBreakMode = GTUILineBreakByTruncatingTail;
                }

                switch (lineBreakMode) {
                    case GTUILineBreakByTruncatingHead:
                        truncationType = kCTLineTruncationStart;
                        break;
                    case GTUILineBreakByTruncatingMiddle:
                        truncationType = kCTLineTruncationMiddle;
                        truncationAttributePosition += (lastLineRange.length / 2);
                        break;
                    case GTUILineBreakByTruncatingTail:
                    default:
               ...
               context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            CGColorRef strokeColor = CGColorRefFromColor([attributes objectForKey:kGTUIBackgroundStrokeColorAttributeName]);
            CGColorRef fillColor = CGColorRefFromColor([attributes objectForKey:kGTUIBackgroundFillColorAttributeName]);
            UIEdgeInsets fillPadding = [[attributes objectForKey:kGTUIBackgroundFillPaddingAttributeName] UIEdgeInsetsValue];
            CGFloat cornerRadius = [[attributes objectForKey:kGTUIBackgroundCornerRadiusAttributeName] floatValue];
            CGFloat lineWidth = [[attributes objectForKey:kGTUIBackgroundLineWidthAttributeName] floatValue];
           ...
           context:(CGContextRef)c
{
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            NSDictionary *attributes = (__bridge NSDictionary *)CTRunGetAttributes((__bridge CTRunRef) glyphRun);
            BOOL strikeOut = [[attributes objectForKey:kGTUIStrikeOutAttributeName] boolValue];
...
#pragma mark - GTUIAttributedLabel
...
- (void)setActiveLink:(GTUIAttributedLabelLink *)activeLink {
     ...
     limitedToNumberOfLines:(NSInteger)numberOfLines
{
    ...
    if (textSize.height < bounds.size.height) {
        ...
        switch (self.verticalAlignment) {
            case GTUIAttributedLabelVerticalAlignmentCenter:
                yOffset = CGFloat_floor((bounds.size.height - textSize.height) / 2.0f);
                break;
            case GTUIAttributedLabelVerticalAlignmentBottom:
                yOffset = bounds.size.height - textSize.height;
                break;
            case GTUIAttributedLabelVerticalAlignmentTop:
            default:
                break;
        }
...
- (void)drawTextInRect:(CGRect)rect {
    ...
    if (self.adjustsFontSizeToFitWidth && self.numberOfLines > 0) {
        ...
        CGSize maxSize = (self.numberOfLines > 1) ? CGSizeMake(GTUIFLOAT_MAX, GTUIFLOAT_MAX) : CGSizeZero;

        CGFloat textWidth = [self sizeThatFits:maxSize].width;
        CGFloat availableWidth = self.frame.size.width * self.numberOfLines;
        if (self.numberOfLines > 1 && self.lineBreakMode == GTUILineBreakByWordWrapping) {
            textWidth *= kGTUILineBreakWordWrapTextWidthScalingFactor;
        }
...
- (NSArray *)accessibilityElements {
    if (!_accessibilityElements) {
        @synchronized(self) {
            ...
            for (GTUIAttributedLabelLink *link in self.linkModels) {
                ...
                if (accessibilityLabel) {
                    GTUIAccessibilityElement *linkElement = [[GTUIAccessibilityElement alloc] initWithAccessibilityContainer:self];
            ...
            GTUIAccessibilityElement *baseElement = [[GTUIAccessibilityElement alloc] initWithAccessibilityContainer:self];
...
- (void)longPressGestureDidFire:(UILongPressGestureRecognizer *)sender {
    switch (sender.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            GTUIAttributedLabelLink *link = [self linkAtPoint:touchPoint];
...
#pragma mark - GTUIAttributedLabelLink

@implementation GTUIAttributedLabelLink
...
- (instancetype)initWithAttributesFromLabel:(GTUIAttributedLabel*)label
                         textCheckingResult:(NSTextCheckingResult *)result {
...
static inline NSDictionary * convertNSAttributedStringAttributesToCTAttributes(NSDictionary *attributes) {
    ...
    NSDictionary *NSToCTAttributeNamesMap = @{
                                              ...
                                              NSBackgroundColorAttributeName: (NSString *)kGTUIBackgroundFillColorAttributeName,
