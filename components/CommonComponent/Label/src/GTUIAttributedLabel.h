//
//  GTUIAttributedLabel.h
//  GTCatalog
//
//  Created by liuxc on 2018/11/6.
//
//  基于TTTAttributedLabel修改富文本标签

#import <UIKit/UIKit.h>
#import <CoreText/CoreText.h>

@class GTUIAttributedLabelLink;

/**
 Vertical alignment for text in a label whose bounds are larger than its text bounds
 */
typedef NS_ENUM(NSInteger, GTUIAttributedLabelVerticalAlignment) {
    GTUIAttributedLabelVerticalAlignmentCenter   = 0,
    GTUIAttributedLabelVerticalAlignmentTop      = 1,
    GTUIAttributedLabelVerticalAlignmentBottom   = 2,
};
...
@protocol GTUIAttributedLabelDelegate;

// Override UILabel @property to accept both NSString and NSAttributedString
@protocol GTUIAttributedLabel <NSObject>
...
/**
 `GTUIAttributedLabel` is a drop-in replacement for `UILabel` that supports `NSAttributedString`, as well as automatically-detected and manually-added links to URLs, addresses, phone numbers, and dates.

 ## Differences Between `GTUIAttributedLabel` and `UILabel`

 For the most part, `GTUIAttributedLabel` behaves just like `UILabel`. The following are notable exceptions, in which `GTUIAttributedLabel` may act differently:
 ...
 `GTUIAttributedLabel`, like `UILabel`, conforms to `NSCoding`. However, if the build target is set to less than iOS 6.0, `linkAttributes` and `activeLinkAttributes` will not be encoded or decoded. This is due to an runtime exception thrown when attempting to copy non-object CoreText values in dictionaries.
...
@interface GTUIAttributedLabel : UILabel <GTUIAttributedLabel, UIGestureRecognizerDelegate>
...
/**
 ...
 @discussion A `GTUIAttributedLabel` delegate responds to messages sent by tapping on links in the label. You can use the delegate to respond to links referencing a URL, address, phone number, date, or date with a specified time zone and duration.
 */
@property (nonatomic, unsafe_unretained) IBOutlet id <GTUIAttributedLabelDelegate> delegate;
...
/**
 The vertical text alignment for the label, for when the frame size is greater than the text rect size. The vertical alignment is `GTUIAttributedLabelVerticalAlignmentCenter` by default.
 */
@property (nonatomic, assign) GTUIAttributedLabelVerticalAlignment verticalAlignment;
...
/**
 Adds a link. You can customize an individual link's appearance and accessibility value by creating your own @c GTUIAttributedLabelLink and passing it to this method. The other methods for adding links will use the label's default attributes.

 @warning Modifying the link's attribute dictionaries must be done before calling this method.

 @param link A @c GTUIAttributedLabelLink object.
 */
...
/**
 Returns the @c GTUIAttributedLabelLink at the give point if it exists.
...
/**
 The `GTUIAttributedLabelDelegate` protocol defines the messages sent to an attributed label delegate when links are tapped. All of the methods of this protocol are optional.
 */
@protocol GTUIAttributedLabelDelegate <NSObject>
 ...
 */
- (void)attributedLabel:(GTUIAttributedLabel *)label
...
@interface GTUIAttributedLabelLink : NSObject <NSCoding>

typedef void (^GTUIAttributedLabelLinkBlock) (GTUIAttributedLabel *, GTUIAttributedLabelLink *);
...
/**
 ...
 @c GTUIAttributedLabelDelegate tap methods, which will not be called for this link.
 */
@property (nonatomic, copy) GTUIAttributedLabelLinkBlock linkTapBlock;

/**
 A block called when this link is long-pressed.
 If non-nil, long pressing on this link will call this block instead of the
 @c GTUIAttributedLabelDelegate long press methods, which will not be called for this link.
 */
@property (nonatomic, copy) GTUIAttributedLabelLinkBlock linkLongPressBlock;
