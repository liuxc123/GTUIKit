//
//  GTUIFormLeftRightSelectorCell.m
//  GTForm
//
...
@implementation GTUIFormLeftRightSelectorCell
{
...
-(UIButton *)leftButton
{
    ...
    _leftButton = [[GTUIFormRightImageButton alloc] init];
    [_leftButton setTranslatesAutoresizingMaskIntoConstraints:NO];
    UIImageView * imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"GTUIForm.bundle/forwardarrow.png"]];
...
-(GTUIFormLeftRightSelectorOption *)leftOptionForDescription:(NSString *)description
{
    if (description){
        return [[self.rowDescriptor.selectorOptions filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(id evaluatedObject, NSDictionary *bindings) {
            return ([description isEqual:[((GTUIFormLeftRightSelectorOption *)evaluatedObject).leftValue displayText]]);
        }]] firstObject];
    }
...
-(void)update
{
    ...
    self.accessoryView = self.rowDescriptor.isDisabled ? nil : [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"GTUIForm.bundle/forwardarrow.png"]];
...
-(void)formDescriptorCellDidSelectedWithFormController:(GTUIFormViewController *)controller
{
    if (self.rowDescriptor.leftRightSelectorLeftOptionSelected){
        ...
        if (option.rightOptions){
            GTUIFormOptionsViewController * optionsViewController = [[GTUIFormOptionsViewController alloc]  initWithStyle:UITableViewStyleGrouped];
            ...
            UIViewController<GTUIFormRowDescriptorViewController> *selectorViewController = [[selectorClass alloc] init];
...
- (id) chooseNewRightValueFromOption:(GTUIFormLeftRightSelectorOption*)option
{
    switch (option.leftValueChangePolicy) {
        case GTUIFormLeftRightSelectorOptionLeftValueChangePolicyChooseLastOption:
            return [option.rightOptions lastObject];
        case GTUIFormLeftRightSelectorOptionLeftValueChangePolicyChooseFirstOption:
            return [option.rightOptions firstObject];
        case GTUIFormLeftRightSelectorOptionLeftValueChangePolicyNullifyRightValue:
            return nil;
    }
    return nil;
}
...
-(void)leftButtonPressed:(UIButton *)leftButton
{
    ...
    for (GTUIFormLeftRightSelectorOption * leftOption in self.rowDescriptor.selectorOptions) {
