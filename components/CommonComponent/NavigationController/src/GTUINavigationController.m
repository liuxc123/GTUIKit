...
@interface GTUINavigationControllerInfo: NSObject

@property(nonatomic, strong) GTUIAppBar *appBar;
...
@implementation GTUINavigationControllerInfo
...
- (UIViewController *)childViewControllerForStatusBarStyle {
    ...
    GTUIAppBar *appBar = [self appBarForViewController:child];
    if (appBar) {
...
- (void)injectAppBarIntoViewController:(UIViewController *)viewController {
    ...
    GTUIAppBar *appBar = [[GTUIAppBar alloc] init];
    ...
    //    unregistered.
    GTUINavigationControllerInfo *info = [[GTUINavigationControllerInfo alloc] init];
    info.appBar = appBar;
    ...
    if ([self.delegate respondsToSelector:
         @selector(navigationController:willAddAppBarViewController:asChildOfViewController:)]) {
...
- (BOOL)viewControllerHasFlexibleHeader:(UIViewController *)viewController {
    ...
    // case. GTUIFlexibleHeaderViewController can never be a top-level view controller.
    for (UIViewController *childViewController in viewController.childViewControllers) {
        if ([childViewController isKindOfClass:[GTUIFlexibleHeaderViewController class]]) {
            return YES;
        }
...
- (GTUINavigationControllerInfo *)infoForViewController:(UIViewController *)viewController {
    return objc_getAssociatedObject(viewController, _cmd);
}

- (void)setInfo:(GTUINavigationControllerInfo *)info forViewController:(UIViewController *)viewController {
...
- (GTUIAppBar *)appBarForViewController:(UIViewController *)viewController {
    return [self infoForViewController:viewController].appBar;
}

- (GTUIAppBarViewController *)naviBarViewControllerForViewController:(UIViewController *)viewController {
