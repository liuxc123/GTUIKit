...
const CGFloat GTUITextInputControllerBaseDefaultBorderRadius = 4.f;
static const CGFloat GTUITextInputControllerBaseDefaultFloatingPlaceholderScaleDefault = 0.75f;
static const CGFloat GTUITextInputControllerBaseDefaultHintTextOpacity = 0.54f;
static const CGFloat GTUITextInputControllerBaseDefaultPadding = 8.f;

static const NSTimeInterval
GTUITextInputControllerBaseDefaultFloatingPlaceholderDownAnimationDuration = 0.266666f;
static const NSTimeInterval
GTUITextInputControllerBaseDefaultFloatingPlaceholderUpAnimationDuration = 0.3f;

static inline UIColor *GTUITextInputControllerBaseDefaultInlinePlaceholderTextColorDefault() {
    return [UIColor colorWithWhite:0 alpha:GTUITextInputControllerBaseDefaultHintTextOpacity];
}

static inline UIColor *GTUITextInputControllerBaseDefaultActiveColorDefault() {
    return [GTUIPalette bluePalette].accent700;
}

static inline UIColor *GTUITextInputControllerBaseDefaultNormalUnderlineColorDefault() {
    return [UIColor lightGrayColor];
}

static inline UIColor *GTUITextInputControllerBaseDefaultTextErrorColorDefault() {
...
GTUITextInputControllerBaseDefaultFloatingPlaceholderScaleDefault;
...
@interface GTUITextInputControllerBase () {
...
@implementation GTUITextInputControllerBase
...
- (instancetype)init {
    self = [super init];
    if (self) {
        if ([self isMemberOfClass:[GTUITextInputControllerBase class]]) {
            NSLog(@"NOTE: GTUITextInputControllerBase is a base class meant to be subclassed.\nFor more"
                  "information, see the README.md file");
        }
        [self commonGTUITextInputControllerBaseInitialization];
    }
...
- (instancetype)copyWithZone:(__unused NSZone *)zone {
    GTUITextInputControllerBase *copy = [[[self class] alloc] init];
...
- (void)commonGTUITextInputControllerBaseInitialization {
...
- (void)setupInput {
    ...
    if ([_textInput conformsToProtocol:@protocol(GTUIMultilineTextInput)] &&
        [_textInput respondsToSelector:@selector(setMinimumLines:)]) {
        ((id<GTUIMultilineTextInput>)_textInput).minimumLines = self.minimumLines;
    }

    if ([_textInput conformsToProtocol:@protocol(GTUIMultilineTextInput)] &&
        [_textInput respondsToSelector:@selector(setExpandsOnOverflow:)]) {
        ((id<GTUIMultilineTextInput>)_textInput).expandsOnOverflow = self.expandsOnOverflow;
    }
...
- (GTUITextInputTextInsetsMode)textInsetModeDefault {
    return GTUITextInputTextInsetsModeAlways;
}
...
- (void)subscribeForNotifications {
    ...
    [defaultCenter addObserver:self
                      selector:@selector(textInputDidToggleEnabled:)
                          name:GTUITextInputDidToggleEnabledNotification
                        object:_textInput];
...
- (UIBezierPath *)defaultBorderPath {
    ...
    return [UIBezierPath
            ...
            cornerRadii:CGSizeMake(GTUITextInputControllerBaseDefaultBorderRadius,
...
- (void)updateLeadingUnderlineLabel {
    ...
    if (self.gtui_adjustsFontForContentSizeCategory) {
        ...
        font = [font gtui_fontSizedForFontTextStyle:GTUIFontTextStyleCaption
...
- (void)updateTextInput {
    ...
    if (self.gtui_adjustsFontForContentSizeCategory) {
        font = [font gtui_fontSizedForFontTextStyle:GTUIFontTextStyleBody1
                                  scaledForDynamicType:_gtui_adjustsFontForContentSizeCategory];
        // TODO: (#4331) This needs to be converted to the new text scheme.
    }
...
- (void)updatePlaceholder {
    ...
    if (![self.textInput.placeholderLabel.font gtui_isSimplyEqual:placeHolderFont]){
...
- (UIOffset)floatingPlaceholderOffset {
    CGFloat vertical = GTUITextInputControllerBaseDefaultPadding;
...
+ (UIColor *)activeColorDefault {
    if (!_activeColorDefault) {
        _activeColorDefault = GTUITextInputControllerBaseDefaultActiveColorDefault();
    }
...
+ (void)setActiveColorDefault:(UIColor *)activeColorDefault {
    ...
    : GTUITextInputControllerBaseDefaultActiveColorDefault();
}
...
+ (UIColor *)disabledColorDefault {
    if (!_disabledColorDefault) {
        _disabledColorDefault = GTUITextInputControllerBaseDefaultNormalUnderlineColorDefault();
    }
...
+ (void)setDisabledColorDefault:(UIColor *)disabledColorDefault {
    ...
    : GTUITextInputControllerBaseDefaultNormalUnderlineColorDefault();
}
...
+ (UIColor *)errorColorDefault {
    if (!_errorColorDefault) {
        _errorColorDefault = GTUITextInputControllerBaseDefaultTextErrorColorDefault();
    }
...
+ (void)setErrorColorDefault:(UIColor *)errorColorDefault {
    ...
    : GTUITextInputControllerBaseDefaultTextErrorColorDefault();
}
...
- (void)setExpandsOnOverflow:(BOOL)expandsOnOverflow {
    if (_expandsOnOverflow != expandsOnOverflow) {
        ...
        if ([_textInput conformsToProtocol:@protocol(GTUIMultilineTextInput)] &&
            [_textInput respondsToSelector:@selector(setExpandsOnOverflow:)]) {
            ((id<GTUIMultilineTextInput>)_textInput).expandsOnOverflow = expandsOnOverflow;
        }
    }
}
...
+ (UIColor *)inlinePlaceholderColorDefault {
    if (!_inlinePlaceholderColorDefault) {
        ...
        GTUITextInputControllerBaseDefaultInlinePlaceholderTextColorDefault();
    }
...
- (void)setMinimumLines:(NSUInteger)minimumLines {
    if (_minimumLines != minimumLines) {
        ...
        if ([_textInput conformsToProtocol:@protocol(GTUIMultilineTextInput)] &&
            [_textInput respondsToSelector:@selector(setMinimumLines:)]) {
            ((id<GTUIMultilineTextInput>)_textInput).minimumLines = minimumLines;
        }
    }
}
...
+ (UIColor *)normalColorDefault {
    if (!_normalColorDefault) {
        _normalColorDefault = GTUITextInputControllerBaseDefaultNormalUnderlineColorDefault();
    }
...
/**
 ...
 GTUITextInputControllerBaseDefaultPadding                             // Top padding
 GTUIRint(self.textInput.placeholderLabel.font.lineHeight * scale)     // Placeholder when up
 GTUITextInputControllerBaseDefaultPadding                             // Padding
 GTUICeil(MAX(self.textInput.font.lineHeight,                          // Text field or placeholder
 self.textInput.placeholderLabel.font.lineHeight))
 GTUITextInputControllerBaseDefaultPadding                             // Padding to underline
...
- (UIEdgeInsets)textInsets:(UIEdgeInsets)defaultInsets {
    ...
    textInsets.top = GTUITextInputControllerBaseDefaultPadding +
    GTUIRint(self.textInput.placeholderLabel.font.lineHeight *
            (CGFloat)self.floatingPlaceholderScale.floatValue) +
    GTUITextInputControllerBaseDefaultPadding;
    ...
    // GTUITextInputControllerBaseDefaultPadding or the biggest estimated underlineLabel height +
    // GTUITextInputControllerBaseDefaultPadding. It's also dependent on the .textInsetsMode.
    CGFloat underlineOffset = 0;
    switch (self.textInput.textInsetsMode) {
        case GTUITextInputTextInsetsModeAlways:
            underlineOffset +=
            MAX(leadingOffset, trailingOffset) + GTUITextInputControllerBaseDefaultPadding;
            break;
        case GTUITextInputTextInsetsModeIfContent: {
            ...
            if (!GTUICGFloatEqual(contentConditionalOffset, 0)) {
                underlineOffset += contentConditionalOffset + GTUITextInputControllerBaseDefaultPadding;
            }
        } break;
    ...
    // .bottom = underlineOffset + GTUITextInputControllerBaseDefaultPadding
    // Legacy default has an additional padding here but this version does not.
    textInsets.bottom = underlineOffset + GTUITextInputControllerBaseDefaultPadding;
...
- (void)textInputDidBeginEditing:(__unused NSNotification *)note {
    [CATransaction begin];
    [CATransaction
     setAnimationDuration:GTUITextInputControllerBaseDefaultFloatingPlaceholderUpAnimationDuration];
    [CATransaction
     setAnimationTimingFunction:[CAMediaTimingFunction
                                 gtui_functionWithType:GTUIAnimationTimingFunctionEaseInOut]];
...
- (void)textInputDidChange:(NSNotification *)note {
    if ([note.name isEqualToString:GTUITextFieldTextDidSetTextNotification]) {
        [CATransaction begin];
...
- (void)textInputDidEndEditing:(__unused NSNotification *)note {
    ...
    [CATransaction setAnimationDuration:
     GTUITextInputControllerBaseDefaultFloatingPlaceholderDownAnimationDuration];
    [CATransaction
     setAnimationTimingFunction:[CAMediaTimingFunction
                                 gtui_functionWithType:GTUIAnimationTimingFunctionEaseInOut]];
