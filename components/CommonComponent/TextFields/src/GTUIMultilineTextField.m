...
@interface GTUIMultilineTextField () {
    UIColor *_cursorColor;

    GTUIIntrinsicHeightTextView *_textView;
}
...
@implementation GTUIMultilineTextField
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _textView = [[GTUIIntrinsicHeightTextView alloc] initWithFrame:CGRectZero];
        [self setupTextView];

        _fundament = [[GTUITextInputCommonFundament alloc] initWithTextInput:self];

        [self commonGTUIMultilineTextFieldInitialization];
    }
...
- (instancetype)copyWithZone:(__unused NSZone *)zone {
    GTUIMultilineTextField *copy = [[[self class] alloc] initWithFrame:self.frame];
...
- (void)commonGTUIMultilineTextFieldInitialization {
    self.backgroundColor = [UIColor clearColor];

    self.textColor = _fundament.textColor;
    // TODO: (#4331) This needs to be converted to the new text scheme.
    self.font = [UIFont gtui_standardFontForTextStyle:GTUIFontTextStyleBody1];
...
- (void)updateConstraints {
    ...
    if (!self.textViewBottomSuperviewBottom) {
        ...
        [NSLayoutConstraint constraintWithItem:self.textView
                                     ...
                                     attribute:NSLayoutAttributeBottom
                                    multiplier:1
                                      constant:-1 * GTUITextInputHalfPadding];
...
- (CGFloat)estimatedTextViewLineHeight {
    ...
    return GTUICeil(self.textView.font.lineHeight * scale) / scale;
}
...
- (void)updateTrailingViewLayout {
    ...
    self.textViewTrailingTrailingViewLeading.active = !GTUICGFloatEqual([self trailingViewAlpha], 0.f);
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:GTUITextFieldTextDidSetTextNotification
                                                        object:self];
}
...
- (GTUIIntrinsicHeightTextView *)textView {
    if (!_textView) {
        _textView = [[GTUIIntrinsicHeightTextView alloc] initWithFrame:CGRectZero];
        [self setupTextView];
        [self setupUnderlineConstraints];
...
- (void)setTextView:(GTUIIntrinsicHeightTextView *)textView {
    ...
    if (currentSize.height != requiredSize.height && self.textView.delegate &&
        [self.layoutDelegate
         respondsToSelector:@selector(multilineTextField:didChangeContentSize:)]) {
            id<GTUIMultilineTextInputLayoutDelegate> delegate =
            (id<GTUIMultilineTextInputLayoutDelegate>)self.layoutDelegate;
