...
#import "private/GTUITextField+Testing.h"
#import "private/GTUITextInputCommonFundament.h"

#import "GTMath.h"
#import "GTTypography.h"

NSString *const GTUITextFieldTextDidSetTextNotification = @"GTUITextFieldTextDidSetTextNotification";
NSString *const GTUITextInputDidToggleEnabledNotification =
@"GTUITextInputDidToggleEnabledNotification";
...
static const CGFloat GTUITextInputClearButtonImageBuiltInPadding = -2.5f;
static const CGFloat GTUITextInputEditingRectRightViewPaddingCorrection = -2.f;
static const CGFloat GTUITextInputTextRectYCorrection = 1.f;

@interface GTUITextField () {
    UIColor *_cursorColor;

    UILabel *_inputLayoutStrut;
}

@property(nonatomic, strong) GTUITextInputCommonFundament *fundament;

/**
 Constraint for center Y of the underline view.

 Default constant: self.top + font line height + GTUITextInputHalfPadding.
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _fundament = [[GTUITextInputCommonFundament alloc] initWithTextInput:self];

        [self commonGTUITextFieldInitialization];
    }
    return self;
}
...
- (instancetype)copyWithZone:(__unused NSZone *)zone {
    GTUITextField *copy = [[[self class] alloc] initWithFrame:self.frame];
...
- (void)commonGTUITextFieldInitialization {
    [super setBorderStyle:UITextBorderStyleNone];

    // Set the clear button color to black with 54% opacity.
    self.clearButton.tintColor = [UIColor colorWithWhite:0 alpha:[GTUITypography captionFontOpacity]];

    _cursorColor = GTUITextInputCursorColor();
...
- (void)setupUnderlineConstraints {
    ...
    [NSLayoutConstraint constraintWithItem:self.underline
     ...
     GTUITextInputHalfPadding];
    _underlineY.priority = UILayoutPriorityDefaultLow;
    _underlineY.active = YES;
}

- (CGFloat)underlineYConstant {
    return [self textInsets].top + [self estimatedTextHeight] + GTUITextInputHalfPadding;
}

- (BOOL)needsUpdateUnderlinePosition {
    return !GTUICGFloatEqual(self.underlineY.constant, [self underlineYConstant]);
}
...
- (UIBezierPath *)defaultBorderPath {
    ...
    return [UIBezierPath
            ...
            cornerRadii:CGSizeMake(GTUITextInputBorderRadius, GTUITextInputBorderRadius)];
}
...
#pragma mark - GTUILeadingViewTextInput Implementation
...
- (GTUITextInputBorderView *)borderView {
    return self.fundament.borderView;
}

- (void)setBorderView:(GTUITextInputBorderView *)borderView {
...
- (UIColor *)cursorColor {
    return _cursorColor ?: GTUITextInputCursorColor();
}
...
- (id<GTUITextInputPositioningDelegate>)positioningDelegate {
    return _fundament.positioningDelegate;
}

- (void)setPositioningDelegate:(id<GTUITextInputPositioningDelegate>)positioningDelegate {
...
- (GTUITextInputTextInsetsMode)textInsetsMode {
...
- (GTUITextInputUnderlineView *)underline {
...
- (void)setAdjustsFontForContentSizeCategory:(BOOL)adjustsFontForContentSizeCategory {
    ...
    [self gtui_setAdjustsFontForContentSizeCategory:adjustsFontForContentSizeCategory];
}
#endif
...
- (void)setEnabled:(BOOL)enabled {
    ...
    [[NSNotificationCenter defaultCenter]
     postNotificationName:GTUITextInputDidToggleEnabledNotification
     object:self];
}
...
- (void)setText:(NSString *)text {
    ...
    if (!self.isFirstResponder) {
        [[NSNotificationCenter defaultCenter]
         postNotificationName:GTUITextFieldTextDidSetTextNotification
         object:self];
    }
}
...
- (CGRect)textRectForBounds:(CGRect)bounds {
    ...
    if (self.rightView.superview) {
        ...
        clearButtonWidth += 2 * GTUITextInputClearButtonImageBuiltInPadding;
    ...
    // Y that works.
    CGFloat actualY =
    (CGRectGetHeight(bounds) / 2.f) - GTUIRint(MAX(self.font.lineHeight,
                                                  self.placeholderLabel.font.lineHeight) /
                                              2.f);  // Text field or placeholder
    actualY = textInsets.top - actualY + GTUITextInputTextRectYCorrection;
...
- (CGRect)editingRectForBounds:(CGRect)bounds {
    ...
    if (self.rightView.superview) {
        editingRect.size.width += GTUITextInputEditingRectRightViewPaddingCorrection;
    } else {
        if (self.text.length > 0) {
            CGFloat clearButtonWidth = CGRectGetWidth(self.clearButton.bounds);
...
- (CGFloat)estimatedTextHeight {
    ...
    CGFloat estimatedTextHeight = GTUICeil(self.font.lineHeight * scale) / scale;
...
- (BOOL)gtui_adjustsFontForContentSizeCategory {
    return _fundament.gtui_adjustsFontForContentSizeCategory;
}
...
- (void)gtui_setAdjustsFontForContentSizeCategory:(BOOL)adjusts {
    ...
    [_fundament gtui_setAdjustsFontForContentSizeCategory:adjusts];
}
