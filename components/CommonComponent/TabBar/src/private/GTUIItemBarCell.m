...
#import "GTUIItemBarCell+Private.h"
...
@implementation GTUIItemBarCell {
    UIImageView *_imageView;
    UILabel *_badgeLabel;
    GTUIInkTouchController *_inkTouchController;
...
+ (CGSize)sizeThatFits:(CGSize)size
   horizontalSizeClass:(UIUserInterfaceSizeClass)sizeClass
    ...
    if (style.shouldDisplayBadge && badge.length > 0) {
        UIFont *badgeFont = [[GTUITypography fontLoader] regularFontOfSize:kBadgeFontSize];
...
- (void)layoutSubviews {
    ...
    _imageView.center = GTUIRoundCenterWithBoundsAndScale(imageCenter, _imageView.bounds, scale);

    _badgeLabel.bounds = GTUIRectAlignToScale(badgeBounds, scale);
    _badgeLabel.center = GTUIRoundCenterWithBoundsAndScale(badgeCenter, _badgeLabel.bounds, scale);

    self.titleLabel.bounds = GTUIRectAlignToScale(titleBounds, scale);
    self.titleLabel.center = GTUIRoundCenterWithBoundsAndScale(titleCenter,
...
+ (NSString *)displayedTitleForTitle:(NSString *)title style:(GTUIItemBarStyle *)style {
...
- (void)updateSubviews {
    ...
    if (_style.shouldDisplayBadge) {
        if (!_badgeLabel) {
            ...
            _badgeLabel.font = [[GTUITypography fontLoader] regularFontOfSize:kBadgeFontSize];
...
- (void)updateTransformsAnimated:(BOOL)animated {
    ...
    if (animated) {
        ...
        [CAMediaTimingFunction gtui_functionWithType:GTUIAnimationTimingFunctionTranslate];
...
- (void)updateInk {
    GTUIInkView *inkView = _inkTouchController.defaultInkView;
    inkView.inkColor = _style.inkColor;
    inkView.inkStyle = _style.inkStyle;
    inkView.usesLegacyInkRipple = NO;
    inkView.clipsToBounds = (inkView.inkStyle == GTUIInkStyleBounded) ? YES : NO;
}
