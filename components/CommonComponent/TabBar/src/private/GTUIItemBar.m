...
static NSString *const kItemReuseID = @"GTUIItem";
...
/// KVO context pointer identifying changes in GTUIItemBarItem properties.
...
@interface GTUIItemBarFlowLayout : UICollectionViewFlowLayout
@end

#pragma mark -

@interface GTUIItemBar () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
@end

@implementation GTUIItemBar {
    // Collection view and layout for items.
    UICollectionView *_collectionView;
    UICollectionViewFlowLayout *_flowLayout;

    /// Indicator layered under the active item.
    GTUITabBarIndicatorView *_selectionIndicator;
...
- (void)commonItemBarInit {
    _alignment = GTUIItemBarAlignmentLeading;
    _style = [[GTUIItemBarStyle alloc] init];
    _items = @[];
    ...
    [collectionView registerClass:[GTUIItemBarCell class] forCellWithReuseIdentifier:kItemReuseID];

    _collectionView = collectionView;
    [self addSubview:_collectionView];

    // Configure the selection indicator view.
    _selectionIndicator = [[GTUITabBarIndicatorView alloc] initWithFrame:CGRectZero];
...
- (void)applyStyle:(GTUIItemBarStyle *)style {
...
- (void)setAlignment:(GTUIItemBarAlignment)alignment {
...
- (id)accessibilityElementForItem:(UITabBarItem *)item {
    ...
    if (index != NSNotFound) {
        ...
        if ([visibleItems containsObject:indexPath]) {
            GTUIItemBarCell *itemCell =
            (GTUIItemBarCell *)[_collectionView cellForItemAtIndexPath:indexPath];
            return itemCell;
        }
    }
    return nil;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == kItemPropertyContext) {
        // GTUIItemBarItem change, must be on the main thread.
        ...
        if (itemIndex != NSNotFound) {
            ...
            if (cell) {
                NSAssert([cell isKindOfClass:[GTUIItemBarCell class]], @"All cells must be GTUIItemBarCell");
                GTUIItemBarCell *itemCell = (GTUIItemBarCell *)cell;
...
shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (_collectionView == collectionView) {
        ...
        id<GTUIItemBarDelegate> delegate = self.delegate;
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    size = [GTUIItemBarCell sizeThatFits:size
                    horizontalSizeClass:[self horizontalSizeClass]
                                   item:item
                                  style:_style];

    // Divide justified items evenly across the view.
    if (_alignment == GTUIItemBarAlignmentJustified) {
...
- (UICollectionViewFlowLayout *)generatedFlowLayout {
    UICollectionViewFlowLayout *flowLayout = [[GTUIItemBarFlowLayout alloc] init];
...
- (void)didSelectItemAtIndex:(NSInteger)index animateTransition:(BOOL)animate {
    ...
    if (animate) {
        ...
        [CAMediaTimingFunction gtui_functionWithType:GTUIAnimationTimingFunctionEaseInOut];
...
- (void)updateSelectionIndicatorToIndex:(NSInteger)index {
    ...
    UICollectionViewCell *cell = [_collectionView cellForItemAtIndexPath:indexPath];
    if ([cell isKindOfClass:[GTUIItemBarCell class]]) {
        GTUIItemBarCell *itemBarCell = (GTUIItemBarCell *)cell;
    ...
    UITabBarItem *item = [self itemAtIndexPath:indexPath];
    GTUITabBarPrivateIndicatorContext *context =
    [[GTUITabBarPrivateIndicatorContext alloc] initWithItem:item
                                                    bounds:selectionIndicatorBounds
                                              contentFrame:contentFrame];

    // Ask the template for attributes.
    id<GTUITabBarIndicatorTemplate> template = _style.selectionIndicatorTemplate;
    GTUITabBarIndicatorAttributes *indicatorAttributes =
...
- (void)updateScrollProperties {
    _collectionView.alwaysBounceHorizontal = (_alignment != GTUIItemBarAlignmentJustified);
}
...
- (void)configureCell:(GTUIItemBarCell *)cell {
...
#pragma mark - GTUIItemBarFlowLayout

@implementation GTUIItemBarFlowLayout {
...
- (BOOL)shouldRelayoutAttributesForRTL {
    ...
    // always be the case for GTUIItemBar, but check for safety.
