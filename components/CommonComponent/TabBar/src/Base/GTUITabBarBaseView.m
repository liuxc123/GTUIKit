...
@interface GTUITabBarBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation GTUITabBarBaseView
...
- (void)setDelegate:(id<GTUITabBarViewDelegate>)delegate {
    _delegate = delegate;
    
    _delegateFlags.didSelectedItemAtIndexFlag = [delegate respondsToSelector:@selector(tabBarView:didSelectedItemAtIndex:)];
    _delegateFlags.didClickSelectedItemAtIndexFlag = [delegate respondsToSelector:@selector(tabBarView:didClickSelectedItemAtIndex:)];
    _delegateFlags.didScrollSelectedItemAtIndexFlag = [delegate respondsToSelector:@selector(tabBarView:didScrollSelectedItemAtIndex:)];
    _delegateFlags.contentScrollViewTransitionToIndexFlag = [delegate respondsToSelector:@selector(tabBarView:contentScrollViewTransitionToIndex:)];
    _delegateFlags.scrollingFromLeftIndexToRightIndexFlag = [delegate respondsToSelector:@selector(tabBarView:scrollingFromLeftIndex:toRightIndex:ratio:)];
}
...
- (void)initializeData
{
    ...
    _selectedIndex = 0;
    _cellWidth = GTUITabBarViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = GTUITabBarViewAutomaticDimension;
    _contentEdgeInsetRight = GTUITabBarViewAutomaticDimension;
...
- (void)initializeViews
{
    self.collectionView = ({
        ...
        GTUITabBarCollectionView *collectionView = [[GTUITabBarCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    GTUITabBarBaseCellModel *cellModel = self.dataSource[index];
    [self refreshCellModel:cellModel index:index];
    GTUITabBarBaseCell *cell = (GTUITabBarBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
...
- (void)refreshState {
    ...
    for (int i = 0; i < self.dataSource.count; i++) {
        GTUITabBarBaseCellModel *cellModel = self.dataSource[i];
        cellModel.index = i;
    ...
    if (self.averageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        if (self.contentEdgeInsetLeft == GTUITabBarViewAutomaticDimension) {
            cellSpacingItemCount += 1;
        }else {
            totalCellSpacingWidth -= self.contentEdgeInsetLeft;
        }
        //如果内容右边距是Automatic，就加1
        if (self.contentEdgeInsetRight == GTUITabBarViewAutomaticDimension) {
            cellSpacingItemCount += 1;
        }else {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(GTUITabBarBaseCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(GTUITabBarBaseCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况、系统会出现GTUITabBarView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现frame为zero时，强行触发布局。
...
- (BOOL)_selectCellAtIndex:(NSInteger)targetIndex handleContentScrollView:(BOOL)handleContentScrollView isClicked:(BOOL)isClicked{
    ...
    if (self.selectedIndex == targetIndex) {
        if (isClicked) {
            if (self.delegateFlags.didClickSelectedItemAtIndexFlag) {
                [self.delegate tabBarView:self didClickSelectedItemAtIndex:targetIndex];
            }
        }else {
            if (self.delegateFlags.didScrollSelectedItemAtIndexFlag) {
                [self.delegate tabBarView:self didScrollSelectedItemAtIndex:targetIndex];
            }
        }
        if (self.delegateFlags.didSelectedItemAtIndexFlag) {
            [self.delegate tabBarView:self didSelectedItemAtIndex:targetIndex];
        }
        return NO;
    }
    
    GTUITabBarBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    GTUITabBarBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
    [self refreshSelectedCellModel:selectedCellModel unselectedCellModel:lastCellModel];
    
    GTUITabBarBaseCell *lastCell = (GTUITabBarBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    
    GTUITabBarBaseCell *selectedCell = (GTUITabBarBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
    ...
    if (handleContentScrollView) {
        if (self.delegateFlags.contentScrollViewTransitionToIndexFlag) {
            [self.delegate tabBarView:self contentScrollViewTransitionToIndex:targetIndex];
        }else {
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.cellWidthZoomEnabled && self.cellWidthZoomScrollGradientEnabled) {
            GTUITabBarBaseCellModel *leftCellModel = (GTUITabBarBaseCellModel *)self.dataSource[baseIndex];
            GTUITabBarBaseCellModel *rightCellModel = (GTUITabBarBaseCellModel *)self.dataSource[baseIndex + 1];
            leftCellModel.cellWidthZoomScale = [GTUITabBarFactory interpolationFrom:self.cellWidthZoomScale to:1.0 percent:remainderRatio];
            rightCellModel.cellWidthZoomScale = [GTUITabBarFactory interpolationFrom:1.0 to:self.cellWidthZoomScale percent:remainderRatio];
            [self.collectionView.collectionViewLayout invalidateLayout];
        }
        
        if (self.delegateFlags.scrollingFromLeftIndexToRightIndexFlag) {
            [self.delegate tabBarView:self scrollingFromLeftIndex:baseIndex toRightIndex:baseIndex + 1 ratio:remainderRatio];
        }
    }
}
...
- (Class)preferredCellClass {
    return GTUITabBarBaseCell.class;
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    GTUITabBarBaseCell *categoryCell = (GTUITabBarBaseCell *)cell;
...
{
    GTUITabBarBaseCellModel *cellModel = self.dataSource[indexPath.item];
