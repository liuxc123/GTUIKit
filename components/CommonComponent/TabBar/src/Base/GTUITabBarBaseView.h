...
@class GTUITabBarBaseView;

@protocol GTUITabBarViewDelegate <NSObject>
 ...
 */
- (void)tabBarView:(GTUITabBarBaseView *)tabBarView contentScrollViewTransitionToIndex:(NSInteger)index;
...
@interface GTUITabBarBaseView : UIView

@property (nonatomic, strong) GTUITabBarCollectionView *collectionView;

@property (nonatomic, strong) NSArray <GTUITabBarBaseCellModel *>*dataSource;

@property (nonatomic, weak) id<GTUITabBarViewDelegate>delegate;
...
@property (nonatomic, assign) CGFloat contentEdgeInsetLeft;     //整体内容的左边距，默认GTUITabBarViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat contentEdgeInsetRight;    //整体内容的右边距，默认GTUITabBarViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat cellWidth;    //默认GTUITabBarViewAutomaticDimension
...
@property (nonatomic, assign) BOOL averageCellSpacingEnabled;     //当item内容总宽度小于GTUITabBarBaseView的宽度，是否将cellSpacing均分。默认为YES。
 ...
 */
- (void)refreshSelectedCellModel:(GTUITabBarBaseCellModel *)selectedCellModel unselectedCellModel:(GTUITabBarBaseCellModel *)unselectedCellModel NS_REQUIRES_SUPER;
