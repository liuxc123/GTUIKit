...
- (void)contentScrollViewDidScrollWithLeftCellFrame:(CGRect)leftCellFrame rightCellFrame:(CGRect)rightCellFrame selectedPosition:(GTUITabBarCellClickedPosition)selectedPosition percent:(CGFloat)percent {
    ...
    if (percent == 0) {
        ...
        if (percent <= 0.5) {
            targetXOfBigBall = [GTUITabBarFactory interpolationFrom:leftX to:(rightX - self.ballScrollOffsetX) percent:percent*2];
            targetWidthOfSmallBall = [GTUITabBarFactory interpolationFrom:self.ballViewSize.width to:self.ballViewSize.width/2 percent:percent*2];
        }else {
            targetXOfBigBall = [GTUITabBarFactory interpolationFrom:(rightX - self.ballScrollOffsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidthOfSmallBall = [GTUITabBarFactory interpolationFrom:self.ballViewSize.width/2 to:0 percent:(percent - 0.5)*2];
            targetXOfSmallBall = [GTUITabBarFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
        }
    }
