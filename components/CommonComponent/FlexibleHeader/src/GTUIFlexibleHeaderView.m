...
#import "private/GTUIFlexibleHeaderMinMaxHeight.h"
#import "private/GTUIFlexibleHeaderTopSafeArea.h"
#import "private/GTUIFlexibleHeaderView+Private.h"
#import "private/GTUIStatusBarShifter.h"
...
// KVO contexts
static char *const kKVOContextGTUIFlexibleHeaderView =
"kKVOContextGTUIFlexibleHeaderView";

static inline GTUIFlexibleHeaderShiftBehavior ShiftBehaviorForCurrentAppContext(
                                                                               GTUIFlexibleHeaderShiftBehavior intendedShiftBehavior) {
    if ([[[NSBundle mainBundle] bundlePath] hasSuffix:@".appex"] &&
        intendedShiftBehavior == GTUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar) {
        return GTUIFlexibleHeaderShiftBehaviorEnabled;
    }
...
@interface GTUIFlexibleHeaderView () <GTUIStatusBarShifterDelegate,
GTUIFlexibleHeaderSafeAreaDelegate,
GTUIFlexibleHeaderMinMaxHeightDelegate>
...
@interface GTUIFlexibleHeaderScrollViewInfo : NSObject
...
@implementation GTUIFlexibleHeaderView {
    ...
    NSMapTable *_trackedScrollViews;  // {UIScrollView:GTUIFlexibleHeaderScrollViewInfo}
    GTUIFlexibleHeaderScrollViewInfo *_trackingInfo;
    ...
    GTUIStatusBarShifter *_statusBarShifter;
    ...
    GTUIFlexibleHeaderShadowIntensityChangeBlock _shadowIntensityChangeBlock;
    ...
    GTUIFlexibleHeaderTopSafeArea *_topSafeArea;
    GTUIFlexibleHeaderMinMaxHeight *_minMaxHeight;
}
...
// GTUIFlexibleHeaderConfiguration properties
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self commonGTUIFlexibleHeaderViewInit];
    }
    return self;
}
...
- (void)commonGTUIFlexibleHeaderViewInit {
    _topSafeArea = [[GTUIFlexibleHeaderTopSafeArea alloc] init];
    _topSafeArea.delegate = self;

    _minMaxHeight = [[GTUIFlexibleHeaderMinMaxHeight alloc] initWithTopSafeArea:_topSafeArea];
    _minMaxHeight.delegate = self;

    _wkWebViewClass = NSClassFromString(@"WKWebView");

    _statusBarShifter = [[GTUIStatusBarShifter alloc] init];
    ...
    _headerContentImportance = GTUIFlexibleHeaderContentImportanceDefault;
    ...
    if (![GTUIFlexibleHeaderView appearance].backgroundColor) {
...
- (void)fhv_setShadowLayer:(CALayer *)shadowLayer
   intensityDidChangeBlock:(GTUIFlexibleHeaderShadowIntensityChangeBlock)block {
...
#pragma mark - GTUIFlexibleHeaderSafeAreas
...
#pragma mark GTUIFlexibleHeaderSafeAreaDelegate

- (void)flexibleHeaderSafeAreaTopSafeAreaInsetDidChange:(GTUIFlexibleHeaderTopSafeArea *)safeAreas {
...
- (BOOL)flexibleHeaderSafeAreaIsStatusBarShifted:(GTUIFlexibleHeaderTopSafeArea *)safeAreas {
    return ([self fhv_canShiftOffscreen] &&
            _shiftBehavior == GTUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar &&
            _statusBarShifter.prefersStatusBarHidden);
}

- (CGFloat)flexibleHeaderSafeAreaDeviceTopSafeAreaInset:(GTUIFlexibleHeaderTopSafeArea *)safeAreas {
    return GTUIDeviceTopSafeAreaInset();
}

#pragma mark - GTUIFlexibleHeaderMinMaxHeightDelegate

- (void)flexibleHeaderMaximumHeightDidChange:(GTUIFlexibleHeaderMinMaxHeight *)safeAreas {
    [self fhv_adjustTrackingScrollViewInsets];
}

- (void)flexibleHeaderMinMaxHeightDidChange:(GTUIFlexibleHeaderMinMaxHeight *)safeAreas {
    [self fhv_updateLayout];
}
...
- (void)fhv_removeInsetsFromScrollView:(UIScrollView *)scrollView {
    ...
    GTUIFlexibleHeaderScrollViewInfo *info = [_trackedScrollViews objectForKey:scrollView];
    if (!info) {
        return;
    }
...
- (CGFloat)fhv_enforceInsetsForScrollView:(UIScrollView *)scrollView {
    ...
    GTUIFlexibleHeaderScrollViewInfo *info = [_trackedScrollViews objectForKey:scrollView];
    if (!info) {
        info = [[GTUIFlexibleHeaderScrollViewInfo alloc] init];
    ...
    BOOL statusBarIsHidden = [UIApplication gtui_safeSharedApplication].statusBarHidden ? YES : NO;
...
- (CGFloat)fhv_accumulatorMax {
    ...
    CGFloat statusBarHeight = [UIApplication gtui_safeSharedApplication].statusBarFrame.size.height;
...
- (BOOL)fhv_canShiftOffscreen {
    return ((_shiftBehavior == GTUIFlexibleHeaderShiftBehaviorEnabled ||
             _shiftBehavior == GTUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar) &&
...
- (void)fhv_recalculatePhase {
    ...
    if (frame.origin.y < 0) {
        _scrollPhase = GTUIFlexibleHeaderScrollPhaseShifting;
        ...
        if ([self fhv_shouldCollapseToStatusBar]) {
            ...
            [UIApplication gtui_safeSharedApplication].statusBarFrame.size.height;
    ...
    if (frame.size.height < _minMaxHeight.maximumHeightWithTopSafeArea) {
        _scrollPhase = GTUIFlexibleHeaderScrollPhaseCollapsing;
    ...
    _scrollPhase = GTUIFlexibleHeaderScrollPhaseOverExtending;
...
- (void)fhv_updateLayout {
    ...
    // GTUIDeviceTopSafeAreaInset returns an incorrect "best guess" value and we end up storing an
...
- (CGFloat)fhv_anchorLength {
    switch (_headerContentImportance) {
        case GTUIFlexibleHeaderContentImportanceDefault:
            return kMaxAnchorLengthFullSwipe;

        case GTUIFlexibleHeaderContentImportanceHigh:
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == kKVOContextGTUIFlexibleHeaderView) {
...
- (void)fhv_startObservingContentOffset {
    [self.trackingScrollView addObserver:self
                              forKeyPath:NSStringFromSelector(@selector(contentOffset))
                                 ...
                                 context:kKVOContextGTUIFlexibleHeaderView];
}
...
- (void)setObservesTrackingScrollViewScrollEvents:(BOOL)observesTrackingScrollViewScrollEvents {
    NSAssert(self.shiftBehavior == GTUIFlexibleHeaderShiftBehaviorDisabled
...
#pragma mark - GTUIStatusBarShifterDelegate

- (void)statusBarShifterNeedsStatusBarAppearanceUpdate:
(__unused GTUIStatusBarShifter *)statusBarShifter {
...
- (void)statusBarShifter:(__unused GTUIStatusBarShifter *)statusBarShifter
...
- (BOOL)hidesStatusBarWhenCollapsed {
    return (_shiftBehavior == GTUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar &&
            !_trackingScrollView.pagingEnabled);
}
...
- (void)setShiftBehavior:(GTUIFlexibleHeaderShiftBehavior)shiftBehavior {
    NSAssert((self.observesTrackingScrollViewScrollEvents
              && shiftBehavior == GTUIFlexibleHeaderShiftBehaviorDisabled)
    ...
    BOOL needsShiftOnScreen = (_shiftBehavior != GTUIFlexibleHeaderShiftBehaviorDisabled &&
                               shiftBehavior == GTUIFlexibleHeaderShiftBehaviorDisabled);
...
- (void)setBehavior:(GTUIFlexibleHeaderShiftBehavior)behavior {
    self.shiftBehavior = behavior;
}

- (GTUIFlexibleHeaderShiftBehavior)behavior {
    return self.shiftBehavior;
}

- (void)changeContentInsets:(GTUIFlexibleHeaderChangeContentInsetsBlock)block {
    if (!block) {
        return;
    }
...
@implementation GTUIFlexibleHeaderScrollViewInfo
@end
