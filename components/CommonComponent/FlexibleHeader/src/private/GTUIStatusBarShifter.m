...
typedef NS_ENUM(NSInteger, GTUIStatusBarShifterState) {
    GTUIStatusBarShifterStateRealStatusBar,
    GTUIStatusBarShifterStateIsSnapshot,
    GTUIStatusBarShifterStateInvalidSnapshot,
};

@implementation GTUIStatusBarShifter {
    ...
    GTUIStatusBarShifterState _snapshotState;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _originalStatusBarHeight = [UIApplication gtui_safeSharedApplication].statusBarFrame.size.height;
...
- (void)invalidateSnapshot {
    [UIView animateWithDuration:kStatusBarBecomesInvalidAnimationDuration
                     animations:^{
                         [self attemptSnapshotState:GTUIStatusBarShifterStateInvalidSnapshot];
                     }];
}
...
- (void)attemptSnapshotState:(GTUIStatusBarShifterState)snapshotState {
    if (_snapshotState == snapshotState) {
        ...
        if (_snapshotState == GTUIStatusBarShifterStateIsSnapshot && [self shouldInvalidateSnapshot]) {
    ...
    if (_snapshotState == GTUIStatusBarShifterStateInvalidSnapshot &&
        snapshotState == GTUIStatusBarShifterStateIsSnapshot) {
        return;
    }

    // Can't go from real => invalid
    if (_snapshotState == GTUIStatusBarShifterStateRealStatusBar &&
        snapshotState == GTUIStatusBarShifterStateInvalidSnapshot) {
        return;
    }

    // While disabled, can't leave the real status bar state.
    if (!_enabled && _snapshotState == GTUIStatusBarShifterStateRealStatusBar) {
        return;
    }

    // If snapshotting is disabled, then can't go from real => snapshot, but must jump to invalid
    // state.
    if (!_snapshottingEnabled && _snapshotState == GTUIStatusBarShifterStateRealStatusBar &&
        snapshotState == GTUIStatusBarShifterStateIsSnapshot) {
        snapshotState = GTUIStatusBarShifterStateInvalidSnapshot;
    }
    ...
    if (_isChangingInterfaceOrientation && snapshotState == GTUIStatusBarShifterStateIsSnapshot) {
        snapshotState = GTUIStatusBarShifterStateInvalidSnapshot;
    }
    ...
    switch (_snapshotState) {
        case GTUIStatusBarShifterStateRealStatusBar: {
        ...
        case GTUIStatusBarShifterStateInvalidSnapshot: {
        ...
        case GTUIStatusBarShifterStateIsSnapshot: {
...
- (void)setOffset:(CGFloat)offset {
    ...
    if (statusOffsetY > 0) {
        ...
        if (_snapshotState == GTUIStatusBarShifterStateInvalidSnapshot) {
            ...
            [self attemptSnapshotState:GTUIStatusBarShifterStateIsSnapshot];
        }
    } else {
        [self attemptSnapshotState:GTUIStatusBarShifterStateRealStatusBar];
    }
}
...
- (BOOL)canUpdateStatusBarFrame {
    CGRect statusBarFrame = [[UIApplication gtui_safeSharedApplication] statusBarFrame];
    CGFloat statusBarHeight = MIN(statusBarFrame.size.width, statusBarFrame.size.height);
    return ((statusBarHeight == _originalStatusBarHeight) || _statusBarReplicaView ||
            _snapshotState == GTUIStatusBarShifterStateInvalidSnapshot);
}
