//
//  GTUIFlexibleHeaderViewController.m
//  GTCatalog
//
...
static NSString *const GTUIFlexibleHeaderViewControllerHeaderViewKey =
@"GTUIFlexibleHeaderViewControllerHeaderViewKey";
static NSString *const GTUIFlexibleHeaderViewControllerLayoutDelegateKey =
@"GTUIFlexibleHeaderViewControllerLayoutDelegateKey";

// KVO contexts
static char *const kKVOContextGTUIFlexibleHeaderViewController =
"kKVOContextGTUIFlexibleHeaderViewController";

@interface GTUIFlexibleHeaderViewController () <GTUIFlexibleHeaderViewDelegate>
...
@implementation GTUIFlexibleHeaderViewController
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        [self commonGTUIFlexibleHeaderViewControllerInit];
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        if ([aDecoder containsValueForKey:GTUIFlexibleHeaderViewControllerHeaderViewKey]) {
            _headerView = [aDecoder decodeObjectOfClass:[GTUIFlexibleHeaderView class]
                                                 forKey:GTUIFlexibleHeaderViewControllerHeaderViewKey];
        }

        if ([aDecoder containsValueForKey:GTUIFlexibleHeaderViewControllerLayoutDelegateKey]) {
            _layoutDelegate =
            [aDecoder decodeObjectForKey:GTUIFlexibleHeaderViewControllerLayoutDelegateKey];
        }
    }
    return self;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    [aCoder encodeObject:self.headerView forKey:GTUIFlexibleHeaderViewControllerHeaderViewKey];
    if (_layoutDelegate) {
        [aCoder encodeConditionalObject:self.layoutDelegate
                                 forKey:GTUIFlexibleHeaderViewControllerLayoutDelegateKey];
    }
}

- (void)commonGTUIFlexibleHeaderViewControllerInit {
    _inferPreferredStatusBarStyle = YES;

    GTUIFlexibleHeaderView *headerView =
    [[GTUIFlexibleHeaderView alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSAssert(![self.parentViewController.parentViewController
               isKindOfClass:[GTUIFlexibleHeaderContainerViewController class]],
             ...
             NSStringFromClass([GTUIFlexibleHeaderContainerViewController class]));
#endif
}
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    if (self.inferPreferredStatusBarStyle) {
        ...
        [GTUIFlexibleHeaderView appearance].backgroundColor ?: _headerView.backgroundColor;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (context == kKVOContextGTUIFlexibleHeaderViewController) {
    ...
    if (shouldObserveLayoutGuideConstant) {
        [_topLayoutGuideConstraint removeObserver:self
                                       forKeyPath:NSStringFromSelector(@selector(constant))
                                          context:kKVOContextGTUIFlexibleHeaderViewController];
    }
    ...
    if (@available(iOS 11.0, *)) {
        ...
        if (self.headerView.trackingScrollView != nil && !alwaysUseAdditionalSafeAreaInsets) {
            ...
            if (self.headerView.minMaxHeightIncludesSafeArea) {
                topInset = MIN(self.headerView.maximumHeight - GTUIDeviceTopSafeAreaInset(), topInset);
            } else {
...
- (CGFloat)headerViewControllerHeight {
    ...
    _headerView.shiftBehavior == GTUIFlexibleHeaderShiftBehaviorEnabledWithStatusBar;
...
#pragma mark GTUIFlexibleHeaderViewDelegate

- (void)flexibleHeaderViewNeedsStatusBarAppearanceUpdate:
(__unused GTUIFlexibleHeaderView *)headerView {
    [self setNeedsStatusBarAppearanceUpdate];
}

- (void)flexibleHeaderViewFrameDidChange:(GTUIFlexibleHeaderView *)headerView {
    if (self.topLayoutGuideAdjustmentEnabled) {
        [self updateTopLayoutGuide];
