...
#import "private/GTUICheckBoxPath.h"
#import "private/GTUICheckBoxAnimations.h"
...
/** The GTUIAnimationManager object used to generate animations.
 */
@property (strong, nonatomic) GTUICheckBoxAnimations *animationManager;

/** The GTUIPathManager object used to generate paths.
 */
@property (strong, nonatomic) GTUICheckBoxPath *pathManager;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self commonInit];
        if (!_boxType) { self.boxType = GTUICheckBoxTypeCircle; }
    }
    return self;
}
...
- (instancetype)initWithType:(GTUICheckBoxType)type {
    GTUICheckBox *checkBox = [self initWithFrame:CGRectMake(0, 0, 30, 30)];
    checkBox.boxType = type;
    return checkBox;
}
...
- (void)commonInit {
    ...
    _onAnimationType = GTUIAnimationTypeStroke;
    _offAnimationType = GTUIAnimationTypeStroke;
...
- (GTUICheckBoxPath *)pathManager {
    if (!_pathManager) {
        _pathManager = [GTUICheckBoxPath new];
...
- (GTUICheckBoxAnimations *)animationManager {
    if (!_animationManager) {
        _animationManager = [[GTUICheckBoxAnimations alloc] initWithAnimationDuration:_animationDuration];
    }
...
- (void)setBoxType:(GTUICheckBoxType)boxType {
    _boxType = boxType;
...
- (void)setOffAnimationType:(GTUIAnimationType)offAnimationType {
...
- (void)drawEntireCheckBox {
    if (!self.hideBox) {
        if (!self.offBoxLayer || CGRectGetHeight(CGPathGetBoundingBox(self.offBoxLayer.path)) == 0.0 || (CGRectGetHeight(self.frame) != self.pathManager.size)) {
            [self drawOffBox];
        }
        if (self.on) {
            [self drawOnBox];
...
- (void)addOnAnimation {
    ...
    switch (self.onAnimationType) {
        ...
        case GTUIAnimationTypeBounce: {
            CGFloat amplitude = (self.boxType == GTUICheckBoxTypeSquare) ? 0.20 : 0.35;
        ...
        case GTUIAnimationTypeOneStroke: {
