...
@implementation GTUIBasePickerView
...
-(UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, ScreenHeight - kGTPickerHeight - kGTTopViewHeight - GTUIDeviceBottomSafeAreaInset() , ScreenWidth, kGTPickerHeight +kGTTopViewHeight + GTUIDeviceBottomSafeAreaInset())];
...
-(UIView *)topView{
    if (!_topView) {
        _topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, self.alertView.frame.size.width, kGTTopViewHeight + 0.5)];
...
-(UILabel *)titleLab{
    if (!_titleLab) {
        _titleLab = [[UILabel alloc]initWithFrame:CGRectMake(self.leftBtn.frame.origin.x + self.leftBtn.frame.size.width + 2, 0, self.alertView.frame.size.width - 2 * (self.leftBtn.frame.origin.x + self.leftBtn.frame.size.width + 2), kGTTopViewHeight)];
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kGTTopViewHeight, self.alertView.frame.size.width, 0.5)];
