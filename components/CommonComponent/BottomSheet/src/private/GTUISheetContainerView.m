//
//  GTUISheetContainerView.m
//  GTCatalog
//
//  Created by liuxc on 2018/11/23.
//

#import "GTUISheetContainerView.h"
...
@interface GTUISheetContainerView () <GTUIDraggableViewDelegate>

@property(nonatomic) GTUISheetState sheetState;
@property(nonatomic) GTUIDraggableView *sheet;
@property(nonatomic) UIView *contentView;

@property(nonatomic) UIDynamicAnimator *animator;
@property(nonatomic) GTUISheetBehavior *sheetBehavior;
...
@implementation GTUISheetContainerView

+ (void)initialize {
    if (self != [GTUISheetContainerView class]) {
        return;
    }
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        _sheetState = GTUISheetStatePreferred;

        // Don't set the frame yet because we're going to change the anchor point.
        _sheet = [[GTUIDraggableView alloc] initWithFrame:CGRectZero scrollView:scrollView];
        ...
        NSArray *notificationNames = @[
                                       GTUIKeyboardWatcherKeyboardWillShowNotification,
                                       GTUIKeyboardWatcherKeyboardWillHideNotification,
                                       GTUIKeyboardWatcherKeyboardWillChangeFrameNotification
                                       ];
...
- (void)didMoveToWindow {
    ...
    if (self.window) {
        if (!self.sheetBehavior) {
            self.sheetBehavior = [[GTUISheetBehavior alloc] initWithItem:self.sheet];
        }
        [self animatePaneWithInitialVelocity:CGPointZero];
    } else {
...
- (void)updateSheetState {
    ...
    self.sheetState = (currentSheetHeight >= [self maximumSheetHeight] ?
                       GTUISheetStateExtended : GTUISheetStatePreferred);
}
...
- (void)animatePaneWithInitialVelocity:(CGPoint)initialVelocity {
    ...
    __weak GTUISheetContainerView *weakSelf = self;
...
- (void)sheetBehaviorDidUpdate {
    ...
    if (self.sheetState == GTUISheetStateClosed &&
        ...
        self.sheetState = GTUISheetStatePreferred;
    }
}
...
#pragma mark - GTUIDraggableViewDelegate

- (CGFloat)maximumHeightForDraggableView:(__unused GTUIDraggableView *)view {
    return [self maximumSheetHeight];
}

- (BOOL)draggableView:(__unused GTUIDraggableView *)view
...
- (void)draggableView:(__unused GTUIDraggableView *)view
draggingEndedWithVelocity:(CGPoint)velocity {
    GTUISheetState targetState;
    if (self.preferredSheetHeight == [self maximumSheetHeight]) {
        // Cannot be extended, only closed.
        targetState = (velocity.y >= 0 ? GTUISheetStateClosed : GTUISheetStatePreferred);
    } else {
        CGFloat currentSheetHeight = CGRectGetMaxY(self.bounds) - CGRectGetMinY(self.sheet.frame);
        if (currentSheetHeight >= self.preferredSheetHeight) {
            targetState = (velocity.y >= 0 ? GTUISheetStatePreferred : GTUISheetStateExtended);
        } else {
            targetState = (velocity.y >= 0 ? GTUISheetStateClosed : GTUISheetStatePreferred);
        }
    }
...
- (void)draggableViewBeganDragging:(__unused GTUIDraggableView *)view {
    [self.animator removeAllBehaviors];
    self.isDragging = YES;
}

- (void)setSheetState:(GTUISheetState)sheetState {
