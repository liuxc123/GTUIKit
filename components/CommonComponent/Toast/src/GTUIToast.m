...
#define GTUIMainThreadAssert() NSAssert([NSThread isMainThread], @"GTUIToast needs to be accessed on the main thread.");

CGFloat const GTUIProgressTopMaxOffset = -1000000.f;
CGFloat const GTUIProgressBottomMaxOffset = 1000000.f;


static const CGFloat GTUIDefaultPadding = 4.f;
static const CGFloat GTUIDefaultLabelFontSize = 16.f;
static const CGFloat GTUIDefaultDetailsLabelFontSize = 12.f;
...
@interface GTUIToastRoundedButton : UIButton
@end
...
+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    GTUIToast *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    GTUIToast *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [GTUIToast allHUDsForView:view];
    for (GTUIToast *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
+ (GTUIToast *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            GTUIToast *hud = (GTUIToast *)subview;
...
- (void)commonInit {
    ...
    _animationType = GTUIToastAnimationFade;
    _mode = GTUIToastModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(GTUIToastAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == GTUIToastAnimationZoom) {
        type = animatingIn ? GTUIToastAnimationZoomIn : GTUIToastAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == GTUIToastAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == GTUIToastAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == GTUIToastAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == GTUIToastAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    GTUIToastCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<GTUIToastDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    GTUIBackgroundView *backgroundView = [[GTUIBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = GTUIToastBackgroundStyleSolidColor;
    ...
    GTUIBackgroundView *bezelView = [GTUIBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:GTUIDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:GTUIDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [GTUIToastRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:GTUIDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[GTUIRoundProgressView class]];

    GTUIToastMode mode = self.mode;
    if (mode == GTUIToastModeIndeterminate) {
    ...
    else if (mode == GTUIToastModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[GTUIBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == GTUIToastModeDeterminate || mode == GTUIToastModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[GTUIRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == GTUIToastModeAnnularDeterminate) {
            [(GTUIRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == GTUIToastModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == GTUIToastModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[GTUIToast class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[GTUIToast class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[GTUIRoundProgressView class]]) {
        GTUIRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [GTUIRoundProgressView appearanceWhenContainedIn:[GTUIToast class], nil];
#else
        appearance = [GTUIRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[GTUIToast class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((GTUIRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((GTUIRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[GTUIBarProgressView class]]) {
        GTUIBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [GTUIBarProgressView appearanceWhenContainedIn:[GTUIToast class], nil];
#else
        appearance = [GTUIBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[GTUIToast class]]];
#endif
        if (appearance.progressColor == nil) {
            ((GTUIBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((GTUIBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    GTUIBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoGTUIie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? GTUIDefaultPadding : 0.f;
...
- (void)setMode:(GTUIToastMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == GTUIToastModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation GTUIRoundProgressView
...
@implementation GTUIBarProgressView
...
@implementation GTUIBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = GTUIToastBackgroundStyleBlur;
...
- (void)setStyle:(GTUIToastBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    GTUIToastBackgroundStyle style = self.style;
    if (style == GTUIToastBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == GTUIToastBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation GTUIToastRoundedButton
