...
#import "private/UIFont+GTUITypographyPrivate.h"

static id<GTUITypographyFontLoading> gFontLoader = nil;
const CGFloat GTUITypographyStandardOpacity = 0.87f;
const CGFloat GTUITypographySecondaryOpacity = 0.54f;

@implementation GTUITypography

#pragma mark - Font loader access

+ (void)setFontLoader:(id<GTUITypographyFontLoading>)fontLoader {
...
+ (id<GTUITypographyFontLoading>)fontLoader {
...
+ (CGFloat)display4FontOpacity {
    return GTUITypographySecondaryOpacity;
}
...
+ (CGFloat)headlineFontOpacity {
    return GTUITypographyStandardOpacity;
}
...
+ (BOOL)isLargeForContrastRatios:(nonnull UIFont *)font {
    id<GTUITypographyFontLoading> fontLoader = [self fontLoader];
...
+ (id<GTUITypographyFontLoading>)defaultFontLoader {
    return [[GTUISystemFontLoader alloc] init];
}

@end

@interface GTUISystemFontLoader ()
...
@implementation GTUISystemFontLoader
...
- (BOOL)isLargeForContrastRatios:(UIFont *)font {
    ...
    CGFloat GTUIFontWeightMedium = (CGFloat)0.23;
...
#pragma clang diagnostic ignored "-Wpartial-availability"
    if (&UIFontWeightMedium != NULL) {
        GTUIFontWeightMedium = UIFontWeightMedium;
    }
#pragma clang diagnostic pop

    // We treat system font medium as large for accessibility when larger than 14.
    if (font.gtui_weight >= GTUIFontWeightMedium) {
        return YES;
    }
