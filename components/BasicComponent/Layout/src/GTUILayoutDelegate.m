...
@implementation GTUIBorderlineLayerDelegate
{
...
-(void)updateBorderLayer:(CAShapeLayer**)ppLayer withBorderline:(GTUIBorderline*)borderline
{
...
-(void)layoutSublayersOfLayer:(CAShapeLayer *)layer
{
    ...
    if (!_gtuiCGRectEqual(layer.frame, layerRect))
    {
...
//布局的事件处理委托对象
@implementation GTUITouchEventDelegate
{
...
BOOL _hasBegin;
__weak GTUIBaseLayout * _currentLayout;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (_layout != nil && _target != nil && !_forbidTouch && touch.tapCount == 1 && !_hasBegin)
    {
        ...
        [self gtuiSetTouchHighlighted];
        
        _hasDoCancel = NO;
        [self gtuiPerformTouch:_touchDownTarget withAction:_touchDownAction];
    }
}
    ...
    if (_layout != nil && _target != nil && _hasBegin && (_layout == _currentLayout || _currentLayout == nil))
    {
        if (_canCallAction)
        {
            ...
            if (fabs(pt.x - _beginPoint.x) > 2 || fabs(pt.y - _beginPoint.y) > 2)
            {
                ...
                if (!_hasDoCancel)
                {
                    [self gtuiPerformTouch:_touchCancelTarget withAction:_touchCancelAction];
...
-(void)doTargetAction:(UITouch*)touch
{
    ...
    {
        [self gtuiPerformTouch:_target withAction:_action];
...
-(void)gtuiPerformTouch:(id)target withAction:(SEL)action
{
    if (_layout != nil && target != nil && action != nil && self.gtuiActionSender != nil)
    {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
        [target performSelector:action withObject:[self gtuiActionSender]];
#pragma clang diagnostic pop
...
@implementation GTUILayoutTouchEventDelegate
{
...
//恢复触摸时的高亮。
-(void)gtui_resetTouchHighlighted
{
    if (self.highlightedOpacity != 0)
    {
...
-(void)gtui_resetTouchHighlighted2
{
    if (_oldAlpha != 1)
    {
        self.layout.alpha = _oldAlpha;
