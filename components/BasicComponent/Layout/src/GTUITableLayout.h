...
/**
 表格布局行列索引描述扩展对象。
 */
@interface NSIndexPath(GTUITableLayoutEx)
 ...
 */
@interface GTUITableLayout : GTUILinearLayout
...
/**
 ...
 2. 特殊尺寸GTUILayoutSize.wrap: 表示由列子视图决定本行尺寸(垂直表格为行高，水平表格为行宽)，每个列子视图都需要自己设置尺寸(垂直表格为高度，水平表格为宽度)
 3. 特殊尺寸GTUILayoutSize.average 表示均分尺寸(垂直表格为行高 = 总行高/行数，水平表格为行宽 = 总行宽/行数)，列子视图不需要设置尺寸(垂直表格为高度，水平表格为宽度)
 4. 不能设置为GTUILayoutSize.fill。
 
 @note 列能设置的值：
 
 1. 一般常数尺寸 表示列内每个子视图都具有固定尺寸(垂直表格为宽度，水平表格为高度)，这时候子视图可以不必设置尺寸。
 2. 特殊尺寸GTUILayoutSize.fill 表示整列尺寸和父视图一样的尺寸(垂直表格为列宽，水平表格为列高)，每个子视图需要设置自己的尺寸(垂直表格为宽度，水平表格为高度)
 
 3. 特殊尺寸GTUILayoutSize.wrap 表示整列的尺寸由列内所有子视图包裹(垂直表格为列宽，水平表格为列高).每个子视图需要设置自己的尺寸(垂直表格为宽度，水平表格为高度)
 
 4. 特殊尺寸GTUILayoutSize.average 表示整列的尺寸和父视图一样的尺寸(垂直表格为列宽，水平表格为列高)，每列内子视图的尺寸均分(垂直表格 = 列宽/行内子视图数，水平表格 = 行高/列内子视图数)
