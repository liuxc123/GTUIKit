...
/**
 ...
 1.垂直数量约束流式布局
 orientation为GTUIOrientationVert,arrangedCount不为0,支持wrapContentHeight,支持wrapContentWidth,不支持autoArrange。
 ...
 2.垂直内容约束流式布局.
 orientation为GTUIOrientationVert,arrangedCount为0,支持wrapContentHeight,不支持wrapContentWidth,支持autoArrange。
 @code
 垂直内容约束流式布局
 =>
 +-----+-----------+
 ...
 3.水平数量约束流式布局。
 orientation为GTUIOrientationHorz,arrangedCount不为0,支持wrapContentHeight,支持wrapContentWidth,不支持autoArrange。
 ...
 4.水平内容约束流式布局
 orientation为GTUIOrientationHorz,arrangedCount为0,不支持wrapContentHeight,支持wrapContentWidth,支持autoArrange。
 ...
 */
@interface GTUIFlowLayout : GTUIBaseLayout
...
/**
 ...
 1. GTUIOrientationVert 表示排内子视图从左到右(RTL从右到左)水平排列，排与排之间依次从上到下垂直排列。这个方向是默认方向。
 
 2. GTUIOrientationHorz 表示排内子视图从上到下垂直排列，排与排之间依次从左到右(RTL从右到左)水平排列。
 */
@property(nonatomic,assign) IBInspectable GTUIOrientation orientation;
...
/**
 ...
 @code
 GTUIGravityVertTop     顶部对齐
 GTUIGravityVertCenter  垂直居中对齐
 GTUIGravityVertBottom  底部对齐
 GTUIGravityVertFill    两端对齐
 GTUIGravityVertBetween  数量约束垂直流式布局有效，子视图会紧凑进行排列,当设置为这个属性值时，子视图的y轴的位置总是从对应列的上一行的结尾开始，而不是上一行的最高位置开始。
 @endcode
 2. 如果是水平流式布局则表示每排内子视图的左中右对齐方式，这里的对齐基础是以每排中的最宽的子视图为基准。这个属性只支持：
 @code
 GTUIGravityHorzLeft    左边对齐
 GTUIGravityHorzCenter  水平居中对齐
 GTUIGravityHorzRight   右边对齐
 GTUIGravityHorzFill    两端对齐
 GTUIGravityHorzBetween  数量约束垂直流式布局有效，子视图会紧凑进行排列，当设置为这个属性值时，子视图的x轴的位置总是从对应行的上一列的结尾开始，而不是上一列的最宽位置开始。
 @endcode
 @note 如果您想单独设置某个子视图在排内的对齐方式则请使用子视图的扩展属性gtui_alignment。
