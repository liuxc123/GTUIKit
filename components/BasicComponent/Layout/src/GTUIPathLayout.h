...
typedef NS_ENUM(NSUInteger, GTUIPathSpaceType) {
    GTUIPathSpaceTypeFlexed,    //浮动距离，子视图之间的距离根据路径布局的尺寸和子视图的数量而确定。
    GTUIPathSpaceTypeFixed,     //固定距离，就是子视图之间的距离是固定的某个数值。
    GTUIPathSpaceTypeCount,     //固定数量距离，就是子视图之间的距离根据路径布局的尺寸和某个具体的数量而确定。
};
 ...
 */
@interface GTUIPathSpace : NSObject
...
/**距离类型。*/
@property(nonatomic, assign, readonly) GTUIPathSpaceType type;
/**距离的值。*/
 ...
 */
@interface GTUICoordinateSetting : NSObject
...
/**
 ...
 你可以从GTUIPathLayout中派生出一个子类。并实现如下方法：
 @code
 +(Class)layerClass
 {
 ...
 @note 也就是说GTUIPathLayout的派生类返回一个CAShapeLayer，那么系统将自动会在每次布局时将layer的path属性进行赋值操作。
 
 */
@interface GTUIPathLayout : GTUIBaseLayout
 ...
 */
@property(nonatomic, strong, readonly) GTUICoordinateSetting *coordinateSetting;
 ...
 */
@property(nonatomic, strong) GTUIPathSpace *spaceType;


/**
 *设置和获取布局视图中的原点视图，默认是nil。如果设置了原点视图则总会将原点视图作为布局视图中的最后一个子视图。原点视图将会显示在路径的坐标原点中心上，因此原点布局是不会参与在路径中的布局的。因为中心原点视图是布局视图中的最后一个子视图，而GTUIPathLayout重写了AddSubview方法，因此可以正常的使用这个方法来添加子视图。
 */
