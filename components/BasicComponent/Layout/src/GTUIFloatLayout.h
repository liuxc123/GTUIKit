...
@interface UIView(GTUIFloatLayoutExt)
 ...
 */
@interface GTUIFloatLayout : GTUIBaseLayout


/**
 初始化一个浮动布局并指定布局的方向。当方向设置为GTUIOrientationVert时表示为左右浮动布局视图，而设置为GTUIOrientationHorz则表示为上下浮动布局视图。
...
/**
 ...
 1.GTUIOrientationVert 表示里面的子视图可以进行左右的浮动，但整体从上到下进行排列的布局方式，这个方式是默认方式。
 
 2.GTUIOrientationHorz 表示里面的子视图可以进行上下的浮动，但整体从左到右进行排列的布局方式，这个方式是默认方式。
 */
...
/**
 ...
 1. 当布局的orientation为GTUIOrientationVert并且wrapContentWidth为YES时,这个属性设置为YES才生效。
 
 2. 当布局的orientation为GTUIOrientationHorz并且wrapContentHeight为YES时，这个属性设置为YES才生效。
