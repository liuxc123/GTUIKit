...
/**
 *视图的布局尺寸类，用来设置视图在布局视图中宽度和高度的尺寸值。布局尺寸类是对尺寸的一个抽象，一个尺寸不一定描述为一个具体的数值，也有可能描述为和另外一个尺寸相等也就是依赖另外一个尺寸，同时一个尺寸可能也会有最大和最小值的限制等等。因此用GTUILayoutSize类来描述这种尺寸的抽象概念。
...
//if you used GTUILayout and Masonry concurrently and you defined MAS_SHORTHAND_GLOBALS in masonry, then you can define GTUI_USEPREFIXMETHOD to solve the conflict.
...
/**
 设置尺寸的具体值，这个具体值可以设置为NSNumber, GTUILayoutSize以及NSArray<GTUILayoutSize*>数组,UIView和nil值。
 
 1. 设置为NSNumber值表示指定具体的宽度或者高度数值
 
 2. 设置为GTUILayoutSize值表示宽度和高度与设置的对象有依赖关系, 甚至可以依赖对象本身
 
 3. 设置为NSArray<GTUILayoutSize*>数组的概念就是所有数组里面的子视图的尺寸平分父视图的尺寸。只有相对布局里面的子视图才支持这种设置。
 
 4. 设置为UIView的概念就是尺寸依赖于指定视图的相对应的尺寸。
 
 5. 设置为nil时则清除设置的具体值。
 */
-(GTUILayoutSize* (^)(__nullable id val ))equalTo;
...
/**
 设置尺寸的最小边界值，如果尺寸对象没有设置最小边界值，那么最小边界默认就是无穷小-CGFLOAT_MAX。lBound方法除了能设置为数值外，还可以设置为GTUILayoutSize值，并且还可以指定增量值和倍数值。
 ...
 sizeVal 指定边界的值。可设置的类型有NSNumber和GTUILayoutSize类型，前者表示最小限制不能小于某个常量值，而后者表示最小限制不能小于另外一个尺寸对象所表示的尺寸值。
...
/**
 设置尺寸的最大边界值，如果尺寸对象没有设置最大边界值，那么最大边界默认就是无穷大CGFLOAT_MAX。uBound方法除了能设置为数值外，还可以设置为GTUILayoutSize值和nil值，并且还可以指定增量值和倍数值。
 ...
 @endcode
 sizeVal 指定边界的值。可设置的类型有NSNumber和GTUILayoutSize类型，前者表示最大限制不能超过某个常量值，而后者表示最大限制不能超过另外一个尺寸对象所表示的尺寸值。
