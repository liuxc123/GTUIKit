...
/*
 几种视图类型的定义：
 1.布局视图：    就是从GTUIBaseLayout派生而出的视图，目前GTUILayout中一共有：线性布局、框架布局、相对布局、表格布局、流式布局、浮动布局、路径布局7种布局。 布局视图也是一个视图。
...
/*
 ...
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Vert GTUILinearLayout |   L,R,CX         | T,B                  | L,R,CX,T,B    |    -      |    -               | L,R   | -     |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Horz GTUILinearLayout |   T,B,CY         | L,R                  | L,R,T,B,CY    |    -      |    -               |  -    |T,B    |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Vert GTUITableLayout  |   T,B,CY         | L,R                  | L,R,T,B,CY    |    -      |    -               |  -    |T,B    |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Horz GTUITableLayout  |   L,R,CX         | T,B                  | L,R,CX,T,B    |    -      |    -               | L,R   |    -    |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |GTUIFrameLayout       |   ALL            |  -                   | L,R,T,B,CX,CY |    -      |    -               |  -    | -     |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |GTUIRelativeLayout    |   ALL            |  -                   |  -            |   ALL     |   CX,CY            | L,R   |T,B    |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Vert GTUIFlowLayout   |   -              | T,B,R,L              |  -            |   -       |   -                | -     | -     |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Horz GTUIFlowLayout   |   -              | T,B,R,L              |  -            |   -       |    -               | -     | -     |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Vert GTUIFloatLayout  |   -              | T,B,R,L              |  -            |   -       |    -               | -     | -     |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |Horz GTUIFloatLayout  |   -              | T,B,R,L              |  -            |   -       |    -               | -     | -     |
 |--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------|
 |GTUIPathLayout        |   -              | T,L                  |  -            |   -       |    -               | -     | -     |
 +--------------------+------------------+----------------------+---------------+-----------+--------------------+-------+-------+
 ...
 *Scene表示在这种场景下支持的布局位置对象的种类，以及设置的值所表达的意义。
 比如上表中的行Vert GTUILinearLayout,列Scene1中的值 L,R,CX 的意思是垂直线性布局下的子视图的leftPos,rightPos,centerXPos的位置值设置为数值时表示的是子视图的左右边距以及水平中心点在父布局下的中心点边距。
 ...
 Scene4:布局位置对象的值设置为GTUILayoutPosition对象，表示某个位置依赖于另外一个位置。只有在相对布局中的子视图的布局位置对象的位置值才能设置为GTUILayoutPosition对象。
 比如： A.leftPos.equalTo(B.rightPos)表示视图A的左边等于视图B的右边。
 
 Scene5:布局位置对象的值设置为NSArray<GTUILayoutPosition>数组对象，表示一组视图整体居中。只有在相对布局中的子视图的centerXPos或者centerYPos中的位置值才可以被设置为这种值。
...
*/
@property(nonatomic, readonly)  GTUILayoutPosition *topPos;

/**
 *视图的头部布局位置对象,对于非阿拉伯国家就是左边，对于阿拉伯国家就是右边(leading layout position of the view.)
 */
@property(nonatomic, readonly)  GTUILayoutPosition *leadingPos;


/**
 *视图的下边布局位置对象。(bottom layout position of the view.)
 */
@property(nonatomic, readonly)  GTUILayoutPosition *bottomPos;


/**
 *视图的尾部布局位置对象,对于非阿拉伯国家就是右边，对于阿拉伯国家就是左边。(trailing layout position of the view.)
 */
@property(nonatomic, readonly)  GTUILayoutPosition *trailingPos;


/**
 *视图的水平中心布局位置对象。(horizontal center layout position of the view.)
 */
@property(nonatomic, readonly)  GTUILayoutPosition *centerXPos;

/**
 *视图的垂直中心布局位置对象。(vertical center layout position of the view.)
 */
@property(nonatomic, readonly)  GTUILayoutPosition *centerYPos;
 ...
 */
@property(nonatomic, readonly)  GTUILayoutPosition *leftPos;

/**
 *视图的右边布局位置对象。如果您不考虑国际化布局就用这个属性，如果考虑国际化则请用trailingPos(right layout position of the view.)
 */
@property(nonatomic, readonly)  GTUILayoutPosition *rightPos;
 ...
 */
@property(nonatomic, readonly) GTUILayoutPosition *baselinePos;


/*
 下面定义的八个属性是上面定义的leftPos,topPos,rightPos,bottomPos,centerXPos,centerYPos的equalTo设置NSNumber类型的值时的简化版本。
 比如：
 v.gtui_left = 10    <==>   v.leftPos.equalTo(@10)
 v.gtui_right = 20   <==>   v.rightPos.equalTo(@20)
 v.gtui_centerX = 0  <==>   v.centerXPos.equalTo(@0)
 
 
 这八个属性的值最好别用于读取，而只是单纯用于设置
 
 这要再次区分一下边距和间距和概念，所谓边距是指子视图距离父视图的距离；而间距则是指子视图距离兄弟视图的距离。gtui_left,gtui_right,gtui_top,gtui_Bottom这几个子视图的扩展属性即可用来表示边距也可以用来表示间距，这个要根据子视图所归属的父布局视图的类型而确定：
 
 1.垂直线性布局gtui_LinearLayout中的子视图： gtui_left,gtui_right表示边距，而gtui_top,gtui_Bottom则表示间距。
 2.水平线性布局gtui_LinearLayout中的子视图： gtui_left,gtui_right表示间距，而gtui_top,gtui_Bottom则表示边距。
 3.表格布局中的子视图：                  gtui_left,gtui_right,gtui_top,gtui_Bottom的定义和线性布局是一致的。
 4.框架布局gtui_frameLayout中的子视图：     gtui_left,gtui_right,gtui_top,gtui_Bottom都表示边距。
 5.相对布局gtui_relativeLayout中的子视图：  gtui_left,gtui_right,gtui_top,gtui_Bottom都表示边距。
 6.流式布局gtui_flowLayout中的子视图：      gtui_left,gtui_right,gtui_top,gtui_Bottom都表示间距。
 7.浮动布局gtui_floatLayout中的子视图：     gtui_left,gtui_right,gtui_top,gtui_Bottom都表示间距。
 8.路径布局gtui_PathLayout中的子视图：      gtui_left,gtui_right,gtui_top,gtui_Bottom即不表示间距也不表示边距，它表示自己中心位置的偏移量。
 9.非布局父视图中的布局子视图：           gtui_left,gtui_right,gtui_top,gtui_Bottom都表示边距。
 10.非布局父视图中的非布局子视图：         gtui_left,gtui_right,gtui_top,gtui_Bottom的设置不会起任何作用，因为gtui_Layout已经无法控制了。
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_top;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_leading;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_bottom;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_trailing;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_centerX;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_centerY;

/**
 *视图中心布局位置, 是gtui_centerX,gtui_CenterY方法的简化版本。此属性只用于赋值不用于读取！
 */
/**
 *Center layout position of the view. Equivalent to set gtui_centerX and gtui_CenterY .Only for setting, not for getting.
 */
@property(nonatomic, assign) IBInspectable CGPoint gtui_center;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_left;
 ...
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_right;



/**
 *视图四边的布局位置, 是gtui_leading,gtui_top,gtui_trailing,gtui_Bottom的简化版本。此属性只用于赋值不用于读取！
 */
/**
 *Boundary layout position of the view. Equivalent to gtui_leading,gtui_top,gtui_trailing,gtui_Bottom set to the same number.Only for setting, not for getting.
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_margin;

/**
 *leading and trailing margin of the view to the superview. Equivalent to gtui_leading,gtui_trailing set to the same number.Only for setting, not for getting.
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_horzMargin;

/**
 *top and bottom margin of the view to the superview. Equivalent to gtui_top,gtui_Bottom set to the same number.Only for setting, not for getting.
 */
@property(nonatomic, assign) IBInspectable CGFloat gtui_vertMargin;

/*
 视图的布局尺寸对象GTUILayoutSize,用于设置视图的宽度和高度尺寸。视图可以通过设置frame值来设置自身的尺寸，也可以通过设置widthSize和heightSize来
 设置布局尺寸，通过frame设置的结果会立即生效，而通过widthSize和heightSize设置则会在布局后才生效，如果同时设置了frame值和GTUILayoutSize值则GTUILayoutSize的设置值优先。
 
 其中的equalTo方法可用于设置布局尺寸的具体值：
 1.如果设置为NSNumber则表示布局尺寸是一个具体的数值。比如widthSize.equalTo(@20)表示视图的宽度设置为20个点。
 2.如果设置为GTUILayoutSize则表示布局尺寸依赖于另外一个视图的布局尺寸。比如A.widthSize.equalTo(B.widthSize)表示A的宽度和B的宽度相等
 3.如果设置为NSArray<GTUILayoutSize*>则表示视图和数组里面的视图均分布局视图的宽度或者高度。比如A.widthSize.equalTo(@[B.widthSize,C.widthSize])表示视图A,B,C三个视图均分父视图的宽度。
 4.如果设置为nil则表示取消布局尺寸的值的设置。
 
 其中的add方法可以用于设置布局尺寸的增加值，一般只和equalTo设置为GTUILayoutSize和NSArray时配合使用。比如A.widthSize.equalTo(B.widthSize).add(20)表示视图A的宽度等于视图B的宽度再加上20个点。
 ...
 +-------------+----------+--------------+---------------+------------+--------------+---------------+--------------+----------------------+
 | 对象 \ 值    | NSNumber |A.widthSize   |A.heightSize   |B.widthSize | B.heightSize |    P.widthSize  |P.heightSize  |NSArray<GTUILayoutSize*>|
 +-------------+----------+--------------+---------------+------------+--------------+---------------+--------------+----------------------+
 ...
 */
/**
 the GTUILayoutSize is layout dimension object object of the UIView. used to set the width and height of the View which is in Layout View
 */



/**
 *视图的宽度布局尺寸对象，可以通过其中的euqalTo方法来设置NSNumber,GTUILayoutSize,NSArray<GTUILayoutSize*>,nil这四种值
 */
@property(nonatomic, readonly)  GTUILayoutSize *widthSize;

/**
 *视图的高度布局尺寸对象，可以通过其中的euqalTo方法来设置NSNumber,GTUILayoutSize,NSArray<GTUILayoutSize*>,nil这四种值
 */
@property(nonatomic, readonly)  GTUILayoutSize *heightSize;
...
/*
 ...
 v.gtui_width = 10    <=>   v.widthSize.equalTo(@10)
 v.gtui_height = 20   <=>   v.heightSize.equalTo(@10)
 ...
 */
@property(nonatomic,assign) IBInspectable CGFloat gtui_width;

/**
 *视图的高度布局尺寸,是heightSize.equalTo方法的简化版本。此属性只用于赋值不用于读取！
 */
@property(nonatomic,assign) IBInspectable CGFloat gtui_height;

/**
 *视图的宽度高度布局尺寸,是gtui_width,gtui_height方法的简化版本。此属性只用于赋值不用于读取！
 */
@property(nonatomic,assign) IBInspectable  CGSize  gtui_size;
...
/**
 ...
 @code
 A.gtui_size = CGSizeMake(100,100);
 B.gtui_size = CGSizeMake(100,50);
 C.gtui_size = CGSizeMake(100,200);
...
/**
 ...
 1. GTUIVisibilityVisible     视图可见，等价于hidden = NO
 
 2. GTUIVisibilityInvisible   视图不可见，等价于hidden = YES, 但是会在父布局视图中占位空白区域
 
 3. GTUIVisibilityGone        视图不可见，等价于hidden = YES, 但是不会在父视图中占位空白区域
 ...
 */
@property(nonatomic,copy,nullable) void (^viewLayoutCompleteBlock)(GTUIBaseLayout* layout, UIView *v);
...
/**
 获取视图在某个Size Classes下的GTUILayoutSizeClass对象。视图可以通过得到的GTUILayoutSizeClass对象来设置视图在对应Size Classes下的各种布局约束属性。
...
/**
 获取视图在某个Size Classes下的GTUILayoutSizeClass对象，如果对象不存在则会新建立一个GTUILayoutSizeClass对象，并且其布局约束属性都拷贝自srcSizeClass中的属性，如果对象已经存在则srcSizeClass不起作用，如果srcSizeClass本来就不存在则也不会起作用。
...
/**
 ...
 GTUILayout布局库根据实际中常见的场景实现了7种不同的布局视图派生类他们分别是：线性布局、表格布局、相对布局、框架布局、流式布局、浮动布局、路径布局。
 */
...
/**
 指定padding内边距的缩进是在SafeArea基础之上进行的。默认是UIRectEdgeLeft|UIRectEdgeRight 表示缩进SafeArea所指定的水平区域。你也可以设置只缩进某一个或则几个方向，或者不缩进任何一个方向。这个属性是为了支持iPoneX而设置的。为了支持iPhoneX的全屏幕适配。我们只需要对根布局视图设置这个扩展属性，默认情况下是不需要进行特殊设置的，GTUILayout自动会对iPhoneX进行适配。我们知道iOS11中引入了安全区域的概念，GTUILayout中的根布局视图会自动将安全区域叠加到设置的padding中去。默认情况下四周的安全区域都会叠加到padding中去，因此您可以根据特殊情况来设置只需要叠加哪一个方向的安全区域。
 */
...
/**
 ...
 1. GTUIGravityVertTop,GTUIGravityVertCenter,GTUIGravityVertBottom 表示整体垂直居上，居中，居下 (支持：框架布局,线性布局,表格布局,流式布局,垂直浮动布局)
 
 2. GTUIGravityHorzLeft,GTUIGravityHorzCenter,GTUIGravityHorzRight 表示整体水平居左，居中，居右 (支持：框架布局,线性布局,表格布局,流式布局,水平浮动布局)
 
 3. GTUIGravityVertBetween 表示每行之间的行间距都被拉伸，以便使里面的子视图垂直方向填充满整个布局视图。 (支持：垂直线性布局,垂直表格布局，流式布局)
 
 4. GTUIGravityHorzBetween 表示每列之间的列间距都被拉伸，以便使里面的子视图水平方向填充满整个布局视图。 (支持：水平线性布局,水平表格布局，流式布局)
 
 5. GTUIGravityVertFill 表示布局会拉伸子视图的高度，以便使里面的子视图垂直方向填充满整个布局视图的高度或者子视图平分布局视图的高度。(支持：框架布局，水平线性布局，水平表格布局，流式布局)
 
 6. GTUIGravityHorzFill 表示布局会拉伸子视图的宽度，以便使里面的子视图水平方向填充满整个布局视图的宽度或者子视图平分布局视图的宽度。 (支持：框架布局，垂直线性布局，垂直表格布局，流式布局)
 7. GTUIGravityVertBaseline 表示布局里面的子视图都基线对齐，目前只支持水平线性布局。
 */
@property(nonatomic, assign) IBInspectable GTUIGravity gravity;
...
/**
 *把一个布局视图放入到UIScrollView(UITableView和UICollectionView除外)内时是否自动调整UIScrollView的contentSize值。默认是GTUIAdjustScrollViewContentSizeModeAuto表示布局视图会自动接管UIScrollView的contentSize的值。 你可以将这个属性设置GTUIAdjustScrollViewContentSizeModeNo而不调整和控制contentSize的值，设置为GTUIAdjustScrollViewContentSizeModeYes则一定会调整contentSize.
 */
@property(nonatomic, assign) GTUIAdjustScrollViewContentSizeMode adjustScrollViewContentSizeMode;
 ...
 */
@property(nonatomic,copy,nullable) void (^beginLayoutBlock)(void);
/**
 ...
 */
@property(nonatomic,copy,nullable) void (^endLayoutBlock)(void);
 ...
 */
@property(nonatomic,copy) void (^rotationToDeviceOrientationBlock)(GTUIBaseLayout *layout, BOOL isFirst, BOOL isPortrait);


/**
 *判断当前是否正在布局中,如果正在布局中则返回YES,否则返回NO
 */
@property(nonatomic,assign,readonly) BOOL isGTUILayouting;


/**
 *设置布局视图的四周的边界线对象。boundBorderLine是同时设置四周的边界线。您也可以单独设置布局视图某些边的边界线。边界线对象默认是nil，您必须通过建立边界线类GTUIBorderline的对象实例并赋值给下面的属性来实现边界线的指定。
 */

/**顶部边界线*/
@property(nonatomic, strong, nullable) GTUIBorderline *topBorderline;
/**头部边界线*/
@property(nonatomic, strong, nullable) GTUIBorderline *leadingBorderline;
/**底部边界线*/
@property(nonatomic, strong, nullable) GTUIBorderline *bottomBorderline;
/**尾部边界线*/
@property(nonatomic, strong, nullable) GTUIBorderline *trailingBorderline;
/**四周边界线*/
@property(nonatomic, strong, nullable) GTUIBorderline *boundBorderline;

/**
 *如果您不需要考虑国际化的问题则请用这个属性设置左边边界线，否则用leadingBorderline
 */
@property(nonatomic, strong, nullable) GTUIBorderline *leftBorderline;
/**
 *如果您不需要考虑国际化的问题则请用这个属性设置右边边界线，否则用trailingBorderline
 */
@property(nonatomic, strong, nullable) GTUIBorderline *rightBorderline;
 ...
 */
@property(nonatomic, strong, nullable) GTUIBorderline *intelligentBorderline;
...
/**
 ...
 @param action 事件的处理动作，格式为：-(void)handleAction:(GTUIBaseLayout*)sender。
 */
