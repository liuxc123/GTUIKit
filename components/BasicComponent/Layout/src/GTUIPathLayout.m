...
@implementation GTUIPathSpace

-(GTUIPathSpace *)initWithSpaceType:(GTUIPathSpaceType)type value:(CGFloat)value
{
    self = [super init];
    if (self != nil)
    {
...
+(GTUIPathSpace *)flexed
{
    return [[self alloc] initWithSpaceType:GTUIPathSpaceTypeFlexed value:0];
}

//固定距离，len为长度，每个子视图之间的距离都是len
+(GTUIPathSpace *)fixed:(CGFloat)len
{
    return [[self alloc] initWithSpaceType:GTUIPathSpaceTypeFixed value:len];
}

//数量距离，根据布局视图的尺寸和指定的数量动态决定。
+(GTUIPathSpace *)count:(NSInteger)count
{
    return [[self alloc] initWithSpaceType:GTUIPathSpaceTypeCount value:count];
}
...
@implementation GTUICoordinateSetting
{
    __weak GTUIPathLayout *_pathLayout;
}

-(GTUICoordinateSetting *)initWithPathLayout:(GTUIPathLayout*)pathLayout
{
    self = [super init];
    if (self != nil)
    {
...
{
    if (!_gtuiCGPointEqual(_origin, origin))
    {
        _origin = origin;
        [_pathLayout setNeedsLayout];
    }
...
@implementation GTUIPathLayout
{
    BOOL _hasOriginView;
    GTUICoordinateSetting  *_coordinateSetting;
    GTUIPathSpace *_spaceType;
}
...
-(GTUICoordinateSetting*)coordinateSetting
{
    if (_coordinateSetting == nil)
    {
        _coordinateSetting = [[GTUICoordinateSetting alloc] initWithPathLayout:self];
    }
    
    return _coordinateSetting;
}

-(GTUIPathSpace*)spaceType
{
    if (_spaceType == nil)
    {
        _spaceType = [GTUIPathSpace flexed];
    }
    
    return _spaceType;
}

-(void)setSpaceType:(GTUIPathSpace *)spaceType
{
...
-(void)beginSubviewPathPoint:(BOOL)full
{
    ...
    if (full)
    {
        ...
        self.pathPoints = [self gtuiCalcPoints:self.pathSubviews path:nil pointIndexArray:pointIndexs lsc:self.gtuiCurrentSizeClass];
        self.pointIndexs = pointIndexs;
    }
    else
    {
        self.pathPoints = [self gtuiCalcPoints:self.pathSubviews path:nil pointIndexArray:nil lsc:self.gtuiCurrentSizeClass];
...
-(CGPathRef)createPath:(NSInteger)subviewCount
{
    ...
    if (self.spaceType.type != GTUIPathSpaceTypeFixed)
    {
        [self gtuiCalcPathPoints:retPath pPathLen:NULL subviewCount:-1 pointIndexArray:nil viewSpace:0 lsc:self.gtuiCurrentSizeClass];
    }
    else
    {
        if (subviewCount == -1)
        ...
        [self gtuiCalcPathPoints:retPath pPathLen:NULL subviewCount:subviewCount pointIndexArray:nil viewSpace:self.spaceType.value lsc:self.gtuiCurrentSizeClass];
    }
...
/*
 ...
 GTUIPathLayout *lsc = self.gtuiCurrentSizeClass;
 
 NSArray *pts = nil;
 if (sbs.count > 0)
 {
 if (self.spaceType.type != GTUIPathSpaceFixed)
 {
 CGFloat totalLen = 0;
 NSArray *tempArray = [self gtuiCalcPathPoints:path pPathLen:&totalLen subviewCount:-1 pointIndexArray:nil viewSpace:0 lsc:lsc];
 BOOL bClose = NO;
 if (tempArray.count > 1)
 {
 CGPoint p1 = [tempArray.firstObject CGPointValue];
 CGPoint p2 = [tempArray.lastObject CGPointValue];
 bClose = [self gtuiCalcDistance:p1 with:p2] <=1;
 }
 
 CGFloat viewSpacing = 0;
 NSInteger sbvcount = sbs.count;
 if (self.spaceType.type == GTUIPathSpaceCount)
 ...
 pts = [self gtuiCalcPathPoints:nil pPathLen:NULL subviewCount:sbs.count pointIndexArray:nil viewSpace:viewSpacing lsc:lsc];
 
 }
 else
 {
 pts = [self gtuiCalcPathPoints:path pPathLen:NULL subviewCount:sbs.count pointIndexArray:nil viewSpace:self.spaceType.value lsc:lsc];
 }
 }
...
-(CGSize)calcLayoutRect:(CGSize)size isEstimate:(BOOL)isEstimate pHasSubLayout:(BOOL*)pHasSubLayout sizeClass:(GTUISizeClass)sizeClass sbs:(NSMutableArray*)sbs
{
    ...
    //算路径子视图的。
    sbs = [self gtuiGetLayoutSubviewsFrom:self.pathSubviews];
    ...
    NSArray *pts = [self gtuiCalcPoints:sbs path:path pointIndexArray:nil lsc:lsc];
    ...
    for (int i = 0; i < sbs.count; i++)
    {
        ...
        if (sbvsc.widthSizeInner.dimeRelaVal != nil)
        {
            ...
            {
                rect.size.width = [sbvsc.widthSizeInner measureWith:(selfSize.width - lsc.gtuiLayoutLeftPadding - lsc.gtuiLayoutRightPadding) ];
            }
            else
            {
        ...
        if (sbvsc.heightSizeInner.dimeRelaVal != nil)
        {
            ...
            {
                rect.size.height = [sbvsc.heightSizeInner measureWith:(selfSize.height - lsc.gtuiLayoutTopPadding - lsc.gtuiLayoutBottomPadding) ];
            }
            else
            {
        ...
        if (_gtuiCGFloatLess(CGRectGetMinY(rect), minYPos))
        {
            minYPos = CGRectGetMinY(rect);
        }
        
        if (_gtuiCGFloatGreat(CGRectGetMaxY(rect), maxYPos))
        {
            maxYPos = CGRectGetMaxY(rect);
        }
        
        if (_gtuiCGFloatLess(CGRectGetMinX(rect), minXPos))
        {
            minXPos = CGRectGetMinX(rect);
        }
        
        if (_gtuiCGFloatGreat(CGRectGetMaxX(rect), maxXPos))
        {
    ...
    if (sbv != nil)
    {
        ...
        //位置在原点位置。。
        rect.origin.x = (selfSize.width - lsc.gtuiLayoutLeftPadding - lsc.gtuiLayoutRightPadding)*self.coordinateSetting.origin.x  - rect.size.width *sbv.layer.anchorPoint.x + sbvsc.leftPosInner.absVal + lsc.gtuiLayoutLeftPadding - sbvsc.rightPosInner.absVal;
        rect.origin.y = (selfSize.height - lsc.gtuiLayoutTopPadding - lsc.gtuiLayoutBottomPadding)*self.coordinateSetting.origin.y - rect.size.height * sbv.layer.anchorPoint.y + sbvsc.topPosInner.absVal + lsc.gtuiLayoutTopPadding - sbvsc.bottomPosInner.absVal;
    ...
    {
        maxYPos = lsc.gtuiLayoutTopPadding + lsc.gtuiLayoutBottomPadding;
    }
    ...
    {
        maxXPos = lsc.gtuiLayoutLeftPadding + lsc.gtuiLayoutRightPadding;
    }
...
{
    return [GTUIPathLayoutViewSizeClass new];
}
...
-(CGFloat)gtuiCalcDistance:(CGPoint)pt1 with:(CGPoint)pt2
{
    ...
    while(true)
    {
        ...
        while (arg - startArg < 0.0001)
        {
            ...
            if (!isnan(realXY.x) && !isnan(realXY.y))
            {
                ...
                distance +=  [self gtuiCalcDistance:realXY with:lastXY];
                if ( _gtuiCGFloatGreatOrEqual(distance, viewSpace))
                {
                    if (_gtuiCGFloatLessOrEqual(distance - viewSpace, self.distanceError))
                    {
                        *pLastValidArg = arg;
                        return realXY;
                    }
        ...
        if (_gtuiCGFloatLessOrEqual(step, 0.001))
        {
            *pLastValidArg = arg;
            break;
        }
...
-(void)gtuiCalcPathPointsHelper:(NSMutableArray*)pathPointArray
            ...
            else
            {
                if (subviewCount >= 0)
                {
                    ...
                    {//如果距离超过间距。则需要缩小自变量步长。以便达到最小的间距误差
                        if (_gtuiCGFloatGreatOrEqual(distance - viewSpace, self.distanceError) )
                        {
                ...
                if (pPathLen != NULL)
                {
                    *pPathLen +=  [self gtuiCalcDistance:realXY with:lastXY];
                }
            }
...
/**
 ...
 @return 返回所有路径点
 */
-(NSArray*)gtuiCalcPathPoints:(CGMutablePathRef)showPath
                   pPathLen:(CGFloat*)pPathLen
               subviewCount:(NSInteger)subviewCount
            pointIndexArray:(NSMutableArray*)pointIndexArray
                  viewSpace:(CGFloat)viewSpace
                        lsc:(GTUIPathLayout*)lsc
{
    NSMutableArray *pathPointArray = [NSMutableArray new];
    
    CGFloat selfWidth = CGRectGetWidth(self.bounds) - lsc.gtuiLayoutLeftPadding - lsc.gtuiLayoutRightPadding;
    CGFloat selfHeight = CGRectGetHeight(self.bounds) - lsc.gtuiLayoutTopPadding - lsc.gtuiLayoutBottomPadding;
    ...
    if (self.rectangularEquation != nil)
    {
        ...
        [self gtuiCalcPathPointsHelper:pathPointArray showPath:showPath pPathLen:pPathLen subviewCount:subviewCount pointIndexArray:pointIndexArray viewSpace:viewSpace startArg:startArg endArg:endArg func:^(CGFloat arg){
            ...
            if (!isnan(val))
            {
                if (self.coordinateSetting.isReverse)
                {
                    return CGPointMake(val + selfWidth * self.coordinateSetting.origin.x + lsc.gtuiLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -arg : arg) + selfHeight * self.coordinateSetting.origin.y + lsc.gtuiLayoutTopPadding);
                    
                }
                else
                {
                    return CGPointMake(arg + selfWidth * self.coordinateSetting.origin.x + lsc.gtuiLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -val : val) + selfHeight * self.coordinateSetting.origin.y + lsc.gtuiLayoutTopPadding);
    ...
    else if (self.parametricEquation != nil)
    {
        ...
        [self gtuiCalcPathPointsHelper:pathPointArray showPath:showPath pPathLen:pPathLen subviewCount:subviewCount pointIndexArray:pointIndexArray  viewSpace:viewSpace startArg:startArg endArg:endArg func:^(CGFloat arg){
            ...
            if (!isnan(val.x) && !isnan(val.y))
            {
                if (self.coordinateSetting.isReverse)
                {
                    return CGPointMake(val.y + selfWidth * self.coordinateSetting.origin.x + lsc.gtuiLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -val.x : val.x) + selfHeight * self.coordinateSetting.origin.y + lsc.gtuiLayoutTopPadding);
                    
                }
                else
                {
                    return CGPointMake(val.x + selfWidth * self.coordinateSetting.origin.x + lsc.gtuiLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -val.y : val.y) + selfHeight * self.coordinateSetting.origin.y + lsc.gtuiLayoutTopPadding);
                }
            }
            else
                return CGPointMake(NAN, NAN);
        ...
        [self gtuiCalcPathPointsHelper:pathPointArray showPath:showPath pPathLen:pPathLen subviewCount:subviewCount pointIndexArray:pointIndexArray  viewSpace:viewSpace startArg:startArg endArg:endArg func:^(CGFloat arg){
            ...
            if (!isnan(r))
            {
                if (self.coordinateSetting.isReverse)
                {
                    ...
                    return CGPointMake(r * sin(arg / 180.0 * M_PI) + selfWidth * self.coordinateSetting.origin.x + lsc.gtuiLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -y : y) + selfHeight * self.coordinateSetting.origin.y + lsc.gtuiLayoutTopPadding);
                ...
                else
                {
                    ...
                    return CGPointMake(r * cos(arg / 180 * M_PI) + selfWidth * self.coordinateSetting.origin.x + lsc.gtuiLayoutLeftPadding,
                                       (self.coordinateSetting.isMath ? -y : y) + selfHeight * self.coordinateSetting.origin.y + lsc.gtuiLayoutTopPadding);
                }
            }
            else
...
-(NSArray*)gtuiCalcPoints:(NSArray*)sbs path:(CGMutablePathRef)path pointIndexArray:(NSMutableArray*)pointIndexArray lsc:(GTUIPathLayout*)lsc
{
    if (sbs.count > 0)
    {
        if (self.spaceType.type != GTUIPathSpaceTypeFixed)
        {
            ...
            NSArray *tempArray = [self gtuiCalcPathPoints:path pPathLen:&pathLen subviewCount:-1 pointIndexArray:nil viewSpace:0 lsc:lsc];
            ...
            NSInteger sbvcount = sbs.count;
            if (self.spaceType.type == GTUIPathSpaceTypeCount)  //如果是固定数量则按固定数量来分配间距
                sbvcount = (NSInteger)self.spaceType.value;
            ...
            //有间距后再重新计算一遍
            return [self gtuiCalcPathPoints:nil pPathLen:NULL subviewCount:sbs.count pointIndexArray:pointIndexArray viewSpace:viewSpace lsc:lsc];
            
        }
        else
        {
            return [self gtuiCalcPathPoints:path pPathLen:NULL subviewCount:sbs.count pointIndexArray:pointIndexArray viewSpace:self.spaceType.value lsc:lsc];
        }
    }
