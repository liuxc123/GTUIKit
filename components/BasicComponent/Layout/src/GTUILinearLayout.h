...
/**
 ...
 @endcode
 */
@interface GTUILinearLayout : GTUIBaseLayout
...
/**
 ...
 1. GTUIOrientation_Vert 表示布局内子视图依次从上到下垂直排列。这个方向是默认方向。
 
 2. GTUIOrientation_Horz 表示布局内子视图依次从左到右(RTL从右到左)水平排列。
 */
...
/**
 用来设置当线性布局中的所有子视图的尺寸和间距的和大于线性布局的尺寸时子视图压缩策略。默认值是GTUISubviewsShrinkNone，表示什么不压缩子视图的尺寸和间距。
 ...
 1. GTUISubviewsShrinkNone | GTUISubviewsShrinkSize(这是布局的默认设置属性):
 
 这种情况下即使是固定的视图的尺寸超出也不会进行任何压缩！！！！
 
 2. GTUISubviewsShrinkAverage | GTUISubviewsShrinkSize (平均压缩)
 ...
 3. GTUISubviewsShrinkWeight | GTUISubviewsShrinkSize(按比例压缩)
 ...
 4.GTUISubviewsShrinkAuto (自动压缩，只对水平线性布局有效）
 ...
 @code
 GTUILinearLayout *horzLayout = [GTUILinearLayout linearLayoutWithOrientation:GTUIOrientation_Horz];
 horzLayout.gtui_horzMargin = 0;
 horzLayout.wrapContentHeight = YES;
 horzLayout.subviewSpace = 10;  //二者的最小间距不能小于20
 horzLayout.shrinkType = GTUISubviewsShrinkAuto;
 
 UILabel *A = [UILabel new];
 A.text = @"xxxxxxx";
 A.widthSize.equalTo(A.widthSize); //宽度等于自身内容的宽度，必须要这么设置和 GTUISubviewsShrinkAuto 结合使用。
 ...
 B.text = @"XXXXXXXX";
 B.widthSize.equalTo(B.widthSize); //宽度等于自身内容的宽度，必须要这么设置和 GTUISubviewsShrinkAuto 结合使用。
 B.wrapContentHeight = YES;        //自动换行
 B.leftPos.equalTo(@0.5);         //左边间距是剩余的50%
 [horzLayout addSubview:B];
 @endcode
 
 5. GTUISubviewsShrinkAverage | GTUISubviewsShrinkSpace (平均压缩间距）
 ...
 6. GTUISubviewsShrinkWeight | GTUISubviewsShrinkSpace: (按比重压缩间距)
...
/**
 ...
 这个属性要和gravity属性设置为GTUIGravityVertBaseline配合使用。并且要求这个属性所指定的视图，必须具有font属性。
 目前支持具有font属性的有UILabel，UITextField,UITextView, UIButton几个系统控件。
 */
@property(nonatomic, assign, nullable) UIView *baselineBaseView;
