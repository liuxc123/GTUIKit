...
typedef NS_OPTIONS(NSUInteger, GTFTextAccessibilityOptions) {
    /** No options. */
    GTFTextAccessibilityOptionsNone = 0,

    /** Font size is at least 14pt bold or 18pt normal. */
    GTFTextAccessibilityOptionsLargeFont = 1 << 0,

    /** Do not modify alpha values to find good colors. */
    GTFTextAccessibilityOptionsPreserveAlpha = 1 << 1,

    /** Prefer darker colors to lighter colors. */
    GTFTextAccessibilityOptionsPreferDarker = 1 << 2,

    /** Prefer lighter colors to darker colors. */
    GTFTextAccessibilityOptionsPreferLighter = 1 << 3,

    /** Use enhanced contrast ratios (level AAA) instead of minimum ratios (level AA). */
    GTFTextAccessibilityOptionsEnhancedContrast = 1 << 4,
};

/**
 GTFTextAccessiblity provides methods to compute accessible text colors.
 */
@interface GTFTextAccessibility : NSObject
...
/**
 ...
 If GTFTextAccessibilityOptionsPreserveAlpha is included in the options, then the algorithm will not
 modify the alpha values, which may result in no color being returned at all.

 @param backgroundColor The opaque background color the text will be displayed on.
 @param targetTextAlpha The target alpha of the text.
 @param options A combination of GTFTextAccessibilityOptions values.
...
/**
 ...
 the GTFTextAccessibilityOptionsLargeFont bit if the font size will be greater than or
 equal to 18pt normal or 14pt bold. If in doubt or if the text size can vary, be conservative and do
 not specify GTFTextAccessibilityOptionsLargeFont in the options.

 By default, the first acceptable color in |choices| will be returned. If
 GTFTextAccessibilityOptionsPreferLighter is set, then the lightest acceptable color will be
 returned, and if GTFTextAccessibilityOptionsPreferDarker is set, then the darkest acceptable color
...
/**
 ...
 GTFTextAccessibilityOptionsLargeFont and GTFTextAccessibilityOptionsEnhancedContrast are relevant
